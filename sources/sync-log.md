# Fork Sync Log


## 2026-09-21T09:19:52Z

**Summary**: ✅ 100 synced · ⚠️ 6 failed · ❌ 0 skipped (total 106)

| Repo | Result | Branch |
|---|---|---|
| `skills` | ✅ synced | `main` |
| `superpowers` | ✅ synced | `main` |
| `andrej-karpathy-skills` | ✅ synced | `main` |
| `gstack` | ✅ synced | `main` |
| `everything-claude-code` | ✅ synced | `main` |
| `awesome-claude-code` | ⚠️ can't sync because there are diverging changes; use `--force` to overwrite the destination branch  | `main` |
| `awesome-claude-skills` | ✅ synced | `main` |
| `antigravity-awesome-skills` | ✅ synced | `main` |
| `claude-howto` | ✅ synced | `main` |
| `awesome-agent-skills` | ✅ synced | `main` |
| `awesome-claude-code-subagents` | ✅ synced | `main` |
| `ralph` | ✅ synced | `main` |
| `awesome-claude-skills-travisvn` | ✅ synced | `main` |
| `context-engineering-intro` | ✅ synced | `main` |
| `claude-code-best-practice` | ✅ synced | `main` |
| `how-claude-code-works` | ✅ synced | `main` |
| `pro-workflow` | ✅ synced | `main` |
| `claude-agents` | ✅ synced | `main` |
| `awesome-claude-code-toolkit` | ✅ synced | `main` |
| `claude-code-sub-agents` | ✅ synced | `main` |
| `claude-code-subagents` | ✅ synced | `main` |
| `awesome-claude-code-plugins` | ✅ synced | `main` |
| `awesome-claude-agents` | ✅ synced | `main` |
| `awesome-agent-skills-heilcheng` | ✅ synced | `main` |
| `awesome-claude-code-jqueryscript` | ✅ synced | `main` |
| `awesome-claude` | ✅ synced | `main` |
| `claude-code-hooks-mastery` | ✅ synced | `main` |
| `claude-code-infrastructure-showcase` | ✅ synced | `main` |
| `claude-code-showcase` | ✅ synced | `main` |
| `Continuous-Claude-v3` | ✅ synced | `main` |
| `buildwithclaude` | ✅ synced | `main` |
| `tdd-guard` | ✅ synced | `main` |
| `Claude-Code-Everything-You-Need-to-Know` | ✅ synced | `main` |
| `tweakcc` | ✅ synced | `main` |
| `oh-my-claudecode` | ✅ synced | `main` |
| `awesome-cursorrules` | ✅ synced | `main` |
| `awesome-copilot` | ⚠️ can't sync because there are diverging changes; use `--force` to overwrite the destination branch  | `main` |
| `awesome-copilot-agents` | ✅ synced | `main` |
| `awesome-cursorrules-zh` | ✅ synced | `master` |
| `myclaude` | ✅ synced | `master` |
| `overstory` | ✅ synced | `main` |
| `microsoft-skills` | ✅ synced | `main` |
| `spec-kit` | ✅ synced | `main` |
| `OpenSpec` | ✅ synced | `main` |
| `spec-workflow-mcp` | ✅ synced | `main` |
| `get-shit-done` | ✅ synced | `main` |
| `shotgun` | ✅ synced | `main` |
| `agents` | ✅ synced | `main` |
| `ruflo` | ⚠️ can't sync because there are diverging changes; use `--force` to overwrite the destination branch  | `main` |
| `open-multi-agent` | ✅ synced | `main` |
| `maestro-orchestrate` | ✅ synced | `main` |
| `flow-next` | ✅ synced | `main` |
| `claude-code-workflow` | ✅ synced | `main` |
| `context-engineering-kit` | ✅ synced | `master` |
| `mcp-servers-official` | ✅ synced | `main` |
| `awesome-mcp-servers` | ✅ synced | `main` |
| `Awesome-MCP-ZH` | ✅ synced | `main` |
| `awesome-remote-mcp-servers` | ✅ synced | `main` |
| `awesome-claude-plugins` | ✅ synced | `master` |
| `awesome-mcp-servers-appcypher` | ✅ synced | `main` |
| `agents.md` | ✅ synced | `main` |
| `design.md` | ✅ synced | `main` |
| `awesome-design-md` | ✅ synced | `main` |
| `Prompt-Engineering-Guide` | ✅ synced | `main` |
| `prompts.chat` | ✅ synced | `main` |
| `SwiftAgents` | ✅ synced | `main` |
| `rails_ai_agents` | ✅ synced | `main` |
| `awesome-openclaw-agents` | ✅ synced | `main` |
| `claude-code-templates` | ⚠️ can't sync because there are diverging changes; use `--force` to overwrite the destination branch  | `main` |
| `claude-code-system-prompts` | ✅ synced | `main` |
| `claude-code-ultimate-guide` | ⚠️ can't sync because there are diverging changes; use `--force` to overwrite the destination branch  | `main` |
| `fastmcp` | ✅ synced | `main` |
| `edict` | ✅ synced | `main` |
| `microsoft-agent-framework` | ✅ synced | `main` |
| `swarms` | ✅ synced | `master` |
| `mission-control` | ✅ synced | `main` |
| `skills-mattpocock` | ✅ synced | `main` |
| `claude-mem` | ✅ synced | `main` |
| `oh-my-openagent` | ✅ synced | `dev` |
| `potpie` | ✅ synced | `main` |
| `cc-sdd` | ✅ synced | `main` |
| `gsd-2` | ✅ synced | `main` |
| `github-mcp-server` | ✅ synced | `main` |
| `my-claude-code-setup` | ✅ synced | `master` |
| `claude-token-efficient` | ⚠️ can't sync because there are diverging changes; use `--force` to overwrite the destination branch  | `main` |
| `playwright-mcp` | ✅ synced | `main` |
| `mcp-toolbox` | ✅ synced | `main` |
| `awesome-codex-subagents` | ✅ synced | `main` |
| `Awesome-Context-Engineering` | ✅ synced | `main` |
| `graphify` | ✅ synced | `v8` |
| `Understand-Anything` | ✅ synced | `main` |
| `awesome-openclaw-skills` | ✅ synced | `main` |
| `gsd-core` | ✅ synced | `next` |
| `agent-os` | ✅ synced | `main` |
| `ponytail` | ✅ synced | `main` |
| `agent-skills` | ✅ synced | `main` |
| `last30days-skill` | ✅ synced | `main` |
| `headroom` | ✅ synced | `main` |
| `Figma-Context-MCP` | ✅ synced | `main` |
| `agent-rules-books` | ✅ synced | `main` |
| `open-saas` | ✅ synced | `main` |
| `openclaw-mission-control` | ✅ synced | `master` |
| `awesome-claude-design` | ✅ synced | `main` |
| `mcp-inspector` | ✅ synced | `main` |
| `codebase-memory-mcp` | ✅ synced | `main` |
| `node-typescript-boilerplate` | ✅ synced | `main` |


## 2026-09-14T09:17:17Z

**Summary**: ✅ 100 synced · ⚠️ 6 failed · ❌ 0 skipped (total 106)

| Repo | Result | Branch |
|---|---|---|
| `skills` | ✅ synced | `main` |
| `superpowers` | ✅ synced | `main` |
| `andrej-karpathy-skills` | ✅ synced | `main` |
| `gstack` | ✅ synced | `main` |
| `everything-claude-code` | ✅ synced | `main` |
| `awesome-claude-code` | ⚠️ can't sync because there are diverging changes; use `--force` to overwrite the destination branch  | `main` |
| `awesome-claude-skills` | ✅ synced | `main` |
| `antigravity-awesome-skills` | ✅ synced | `main` |
| `claude-howto` | ✅ synced | `main` |
| `awesome-agent-skills` | ✅ synced | `main` |
| `awesome-claude-code-subagents` | ✅ synced | `main` |
| `ralph` | ✅ synced | `main` |
| `awesome-claude-skills-travisvn` | ✅ synced | `main` |
| `context-engineering-intro` | ✅ synced | `main` |
| `claude-code-best-practice` | ✅ synced | `main` |
| `how-claude-code-works` | ✅ synced | `main` |
| `pro-workflow` | ✅ synced | `main` |
| `claude-agents` | ✅ synced | `main` |
| `awesome-claude-code-toolkit` | ✅ synced | `main` |
| `claude-code-sub-agents` | ✅ synced | `main` |
| `claude-code-subagents` | ✅ synced | `main` |
| `awesome-claude-code-plugins` | ✅ synced | `main` |
| `awesome-claude-agents` | ✅ synced | `main` |
| `awesome-agent-skills-heilcheng` | ✅ synced | `main` |
| `awesome-claude-code-jqueryscript` | ✅ synced | `main` |
| `awesome-claude` | ✅ synced | `main` |
| `claude-code-hooks-mastery` | ✅ synced | `main` |
| `claude-code-infrastructure-showcase` | ✅ synced | `main` |
| `claude-code-showcase` | ✅ synced | `main` |
| `Continuous-Claude-v3` | ✅ synced | `main` |
| `buildwithclaude` | ✅ synced | `main` |
| `tdd-guard` | ✅ synced | `main` |
| `Claude-Code-Everything-You-Need-to-Know` | ✅ synced | `main` |
| `tweakcc` | ✅ synced | `main` |
| `oh-my-claudecode` | ✅ synced | `main` |
| `awesome-cursorrules` | ✅ synced | `main` |
| `awesome-copilot` | ⚠️ can't sync because there are diverging changes; use `--force` to overwrite the destination branch  | `main` |
| `awesome-copilot-agents` | ✅ synced | `main` |
| `awesome-cursorrules-zh` | ✅ synced | `master` |
| `myclaude` | ✅ synced | `master` |
| `overstory` | ✅ synced | `main` |
| `microsoft-skills` | ✅ synced | `main` |
| `spec-kit` | ✅ synced | `main` |
| `OpenSpec` | ✅ synced | `main` |
| `spec-workflow-mcp` | ✅ synced | `main` |
| `get-shit-done` | ✅ synced | `main` |
| `shotgun` | ✅ synced | `main` |
| `agents` | ✅ synced | `main` |
| `ruflo` | ⚠️ can't sync because there are diverging changes; use `--force` to overwrite the destination branch  | `main` |
| `open-multi-agent` | ✅ synced | `main` |
| `maestro-orchestrate` | ✅ synced | `main` |
| `flow-next` | ✅ synced | `main` |
| `claude-code-workflow` | ✅ synced | `main` |
| `context-engineering-kit` | ✅ synced | `master` |
| `mcp-servers-official` | ✅ synced | `main` |
| `awesome-mcp-servers` | ✅ synced | `main` |
| `Awesome-MCP-ZH` | ✅ synced | `main` |
| `awesome-remote-mcp-servers` | ✅ synced | `main` |
| `awesome-claude-plugins` | ✅ synced | `master` |
| `awesome-mcp-servers-appcypher` | ✅ synced | `main` |
| `agents.md` | ✅ synced | `main` |
| `design.md` | ✅ synced | `main` |
| `awesome-design-md` | ✅ synced | `main` |
| `Prompt-Engineering-Guide` | ✅ synced | `main` |
| `prompts.chat` | ✅ synced | `main` |
| `SwiftAgents` | ✅ synced | `main` |
| `rails_ai_agents` | ✅ synced | `main` |
| `awesome-openclaw-agents` | ✅ synced | `main` |
| `claude-code-templates` | ⚠️ can't sync because there are diverging changes; use `--force` to overwrite the destination branch  | `main` |
| `claude-code-system-prompts` | ✅ synced | `main` |
| `claude-code-ultimate-guide` | ⚠️ can't sync because there are diverging changes; use `--force` to overwrite the destination branch  | `main` |
| `fastmcp` | ✅ synced | `main` |
| `edict` | ✅ synced | `main` |
| `microsoft-agent-framework` | ✅ synced | `main` |
| `swarms` | ✅ synced | `master` |
| `mission-control` | ✅ synced | `main` |
| `skills-mattpocock` | ✅ synced | `main` |
| `claude-mem` | ✅ synced | `main` |
| `oh-my-openagent` | ✅ synced | `dev` |
| `potpie` | ✅ synced | `main` |
| `cc-sdd` | ✅ synced | `main` |
| `gsd-2` | ✅ synced | `main` |
| `github-mcp-server` | ✅ synced | `main` |
| `my-claude-code-setup` | ✅ synced | `master` |
| `claude-token-efficient` | ⚠️ can't sync because there are diverging changes; use `--force` to overwrite the destination branch  | `main` |
| `playwright-mcp` | ✅ synced | `main` |
| `mcp-toolbox` | ✅ synced | `main` |
| `awesome-codex-subagents` | ✅ synced | `main` |
| `Awesome-Context-Engineering` | ✅ synced | `main` |
| `graphify` | ✅ synced | `v8` |
| `Understand-Anything` | ✅ synced | `main` |
| `awesome-openclaw-skills` | ✅ synced | `main` |
| `gsd-core` | ✅ synced | `next` |
| `agent-os` | ✅ synced | `main` |
| `ponytail` | ✅ synced | `main` |
| `agent-skills` | ✅ synced | `main` |
| `last30days-skill` | ✅ synced | `main` |
| `headroom` | ✅ synced | `main` |
| `Figma-Context-MCP` | ✅ synced | `main` |
| `agent-rules-books` | ✅ synced | `main` |
| `open-saas` | ✅ synced | `main` |
| `openclaw-mission-control` | ✅ synced | `master` |
| `awesome-claude-design` | ✅ synced | `main` |
| `mcp-inspector` | ✅ synced | `main` |
| `codebase-memory-mcp` | ✅ synced | `main` |
| `node-typescript-boilerplate` | ✅ synced | `main` |


## 2026-09-07T08:39:54Z

**Summary**: ✅ 101 synced · ⚠️ 5 failed · ❌ 0 skipped (total 106)

| Repo | Result | Branch |
|---|---|---|
| `skills` | ✅ synced | `main` |
| `superpowers` | ✅ synced | `main` |
| `andrej-karpathy-skills` | ✅ synced | `main` |
| `gstack` | ✅ synced | `main` |
| `everything-claude-code` | ✅ synced | `main` |
| `awesome-claude-code` | ⚠️ can't sync because there are diverging changes; use `--force` to overwrite the destination branch  | `main` |
| `awesome-claude-skills` | ✅ synced | `main` |
| `antigravity-awesome-skills` | ✅ synced | `main` |
| `claude-howto` | ✅ synced | `main` |
| `awesome-agent-skills` | ✅ synced | `main` |
| `awesome-claude-code-subagents` | ✅ synced | `main` |
| `ralph` | ✅ synced | `main` |
| `awesome-claude-skills-travisvn` | ✅ synced | `main` |
| `context-engineering-intro` | ✅ synced | `main` |
| `claude-code-best-practice` | ✅ synced | `main` |
| `how-claude-code-works` | ✅ synced | `main` |
| `pro-workflow` | ✅ synced | `main` |
| `claude-agents` | ✅ synced | `main` |
| `awesome-claude-code-toolkit` | ✅ synced | `main` |
| `claude-code-sub-agents` | ✅ synced | `main` |
| `claude-code-subagents` | ✅ synced | `main` |
| `awesome-claude-code-plugins` | ✅ synced | `main` |
| `awesome-claude-agents` | ✅ synced | `main` |
| `awesome-agent-skills-heilcheng` | ✅ synced | `main` |
| `awesome-claude-code-jqueryscript` | ✅ synced | `main` |
| `awesome-claude` | ✅ synced | `main` |
| `claude-code-hooks-mastery` | ✅ synced | `main` |
| `claude-code-infrastructure-showcase` | ✅ synced | `main` |
| `claude-code-showcase` | ✅ synced | `main` |
| `Continuous-Claude-v3` | ✅ synced | `main` |
| `buildwithclaude` | ✅ synced | `main` |
| `tdd-guard` | ✅ synced | `main` |
| `Claude-Code-Everything-You-Need-to-Know` | ✅ synced | `main` |
| `tweakcc` | ✅ synced | `main` |
| `oh-my-claudecode` | ✅ synced | `main` |
| `awesome-cursorrules` | ✅ synced | `main` |
| `awesome-copilot` | ⚠️ can't sync because there are diverging changes; use `--force` to overwrite the destination branch  | `main` |
| `awesome-copilot-agents` | ✅ synced | `main` |
| `awesome-cursorrules-zh` | ✅ synced | `master` |
| `myclaude` | ✅ synced | `master` |
| `overstory` | ✅ synced | `main` |
| `microsoft-skills` | ✅ synced | `main` |
| `spec-kit` | ✅ synced | `main` |
| `OpenSpec` | ✅ synced | `main` |
| `spec-workflow-mcp` | ✅ synced | `main` |
| `get-shit-done` | ✅ synced | `main` |
| `shotgun` | ✅ synced | `main` |
| `agents` | ✅ synced | `main` |
| `ruflo` | ⚠️ can't sync because there are diverging changes; use `--force` to overwrite the destination branch  | `main` |
| `open-multi-agent` | ✅ synced | `main` |
| `maestro-orchestrate` | ✅ synced | `main` |
| `flow-next` | ✅ synced | `main` |
| `claude-code-workflow` | ✅ synced | `main` |
| `context-engineering-kit` | ✅ synced | `master` |
| `mcp-servers-official` | ✅ synced | `main` |
| `awesome-mcp-servers` | ✅ synced | `main` |
| `Awesome-MCP-ZH` | ✅ synced | `main` |
| `awesome-remote-mcp-servers` | ✅ synced | `main` |
| `awesome-claude-plugins` | ✅ synced | `master` |
| `awesome-mcp-servers-appcypher` | ✅ synced | `main` |
| `agents.md` | ✅ synced | `main` |
| `design.md` | ✅ synced | `main` |
| `awesome-design-md` | ✅ synced | `main` |
| `Prompt-Engineering-Guide` | ✅ synced | `main` |
| `prompts.chat` | ✅ synced | `main` |
| `SwiftAgents` | ✅ synced | `main` |
| `rails_ai_agents` | ✅ synced | `main` |
| `awesome-openclaw-agents` | ✅ synced | `main` |
| `claude-code-templates` | ⚠️ can't sync because there are diverging changes; use `--force` to overwrite the destination branch  | `main` |
| `claude-code-system-prompts` | ✅ synced | `main` |
| `claude-code-ultimate-guide` | ✅ synced | `main` |
| `fastmcp` | ✅ synced | `main` |
| `edict` | ✅ synced | `main` |
| `microsoft-agent-framework` | ✅ synced | `main` |
| `swarms` | ✅ synced | `master` |
| `mission-control` | ✅ synced | `main` |
| `skills-mattpocock` | ✅ synced | `main` |
| `claude-mem` | ✅ synced | `main` |
| `oh-my-openagent` | ✅ synced | `dev` |
| `potpie` | ✅ synced | `main` |
| `cc-sdd` | ✅ synced | `main` |
| `gsd-2` | ✅ synced | `main` |
| `github-mcp-server` | ✅ synced | `main` |
| `my-claude-code-setup` | ✅ synced | `master` |
| `claude-token-efficient` | ⚠️ can't sync because there are diverging changes; use `--force` to overwrite the destination branch  | `main` |
| `playwright-mcp` | ✅ synced | `main` |
| `mcp-toolbox` | ✅ synced | `main` |
| `awesome-codex-subagents` | ✅ synced | `main` |
| `Awesome-Context-Engineering` | ✅ synced | `main` |
| `graphify` | ✅ synced | `v8` |
| `Understand-Anything` | ✅ synced | `main` |
| `awesome-openclaw-skills` | ✅ synced | `main` |
| `gsd-core` | ✅ synced | `next` |
| `agent-os` | ✅ synced | `main` |
| `ponytail` | ✅ synced | `main` |
| `agent-skills` | ✅ synced | `main` |
| `last30days-skill` | ✅ synced | `main` |
| `headroom` | ✅ synced | `main` |
| `Figma-Context-MCP` | ✅ synced | `main` |
| `agent-rules-books` | ✅ synced | `main` |
| `open-saas` | ✅ synced | `main` |
| `openclaw-mission-control` | ✅ synced | `master` |
| `awesome-claude-design` | ✅ synced | `main` |
| `mcp-inspector` | ✅ synced | `main` |
| `codebase-memory-mcp` | ✅ synced | `main` |
| `node-typescript-boilerplate` | ✅ synced | `main` |


## 2026-08-31T10:07:58Z

**Summary**: ✅ 101 synced · ⚠️ 5 failed · ❌ 0 skipped (total 106)

| Repo | Result | Branch |
|---|---|---|
| `skills` | ✅ synced | `main` |
| `superpowers` | ✅ synced | `main` |
| `andrej-karpathy-skills` | ✅ synced | `main` |
| `gstack` | ✅ synced | `main` |
| `everything-claude-code` | ✅ synced | `main` |
| `awesome-claude-code` | ⚠️ can't sync because there are diverging changes; use `--force` to overwrite the destination branch  | `main` |
| `awesome-claude-skills` | ✅ synced | `main` |
| `antigravity-awesome-skills` | ✅ synced | `main` |
| `claude-howto` | ✅ synced | `main` |
| `awesome-agent-skills` | ✅ synced | `main` |
| `awesome-claude-code-subagents` | ✅ synced | `main` |
| `ralph` | ✅ synced | `main` |
| `awesome-claude-skills-travisvn` | ✅ synced | `main` |
| `context-engineering-intro` | ✅ synced | `main` |
| `claude-code-best-practice` | ✅ synced | `main` |
| `how-claude-code-works` | ✅ synced | `main` |
| `pro-workflow` | ✅ synced | `main` |
| `claude-agents` | ✅ synced | `main` |
| `awesome-claude-code-toolkit` | ✅ synced | `main` |
| `claude-code-sub-agents` | ✅ synced | `main` |
| `claude-code-subagents` | ✅ synced | `main` |
| `awesome-claude-code-plugins` | ✅ synced | `main` |
| `awesome-claude-agents` | ✅ synced | `main` |
| `awesome-agent-skills-heilcheng` | ✅ synced | `main` |
| `awesome-claude-code-jqueryscript` | ✅ synced | `main` |
| `awesome-claude` | ✅ synced | `main` |
| `claude-code-hooks-mastery` | ✅ synced | `main` |
| `claude-code-infrastructure-showcase` | ✅ synced | `main` |
| `claude-code-showcase` | ✅ synced | `main` |
| `Continuous-Claude-v3` | ✅ synced | `main` |
| `buildwithclaude` | ✅ synced | `main` |
| `tdd-guard` | ✅ synced | `main` |
| `Claude-Code-Everything-You-Need-to-Know` | ✅ synced | `main` |
| `tweakcc` | ✅ synced | `main` |
| `oh-my-claudecode` | ✅ synced | `main` |
| `awesome-cursorrules` | ✅ synced | `main` |
| `awesome-copilot` | ⚠️ can't sync because there are diverging changes; use `--force` to overwrite the destination branch  | `main` |
| `awesome-copilot-agents` | ✅ synced | `main` |
| `awesome-cursorrules-zh` | ✅ synced | `master` |
| `myclaude` | ✅ synced | `master` |
| `overstory` | ✅ synced | `main` |
| `microsoft-skills` | ✅ synced | `main` |
| `spec-kit` | ✅ synced | `main` |
| `OpenSpec` | ✅ synced | `main` |
| `spec-workflow-mcp` | ✅ synced | `main` |
| `get-shit-done` | ✅ synced | `main` |
| `shotgun` | ✅ synced | `main` |
| `agents` | ✅ synced | `main` |
| `ruflo` | ⚠️ can't sync because there are diverging changes; use `--force` to overwrite the destination branch  | `main` |
| `open-multi-agent` | ✅ synced | `main` |
| `maestro-orchestrate` | ✅ synced | `main` |
| `flow-next` | ✅ synced | `main` |
| `claude-code-workflow` | ✅ synced | `main` |
| `context-engineering-kit` | ✅ synced | `master` |
| `mcp-servers-official` | ✅ synced | `main` |
| `awesome-mcp-servers` | ✅ synced | `main` |
| `Awesome-MCP-ZH` | ✅ synced | `main` |
| `awesome-remote-mcp-servers` | ✅ synced | `main` |
| `awesome-claude-plugins` | ✅ synced | `master` |
| `awesome-mcp-servers-appcypher` | ✅ synced | `main` |
| `agents.md` | ✅ synced | `main` |
| `design.md` | ✅ synced | `main` |
| `awesome-design-md` | ✅ synced | `main` |
| `Prompt-Engineering-Guide` | ✅ synced | `main` |
| `prompts.chat` | ✅ synced | `main` |
| `SwiftAgents` | ✅ synced | `main` |
| `rails_ai_agents` | ✅ synced | `main` |
| `awesome-openclaw-agents` | ✅ synced | `main` |
| `claude-code-templates` | ⚠️ can't sync because there are diverging changes; use `--force` to overwrite the destination branch  | `main` |
| `claude-code-system-prompts` | ✅ synced | `main` |
| `claude-code-ultimate-guide` | ✅ synced | `main` |
| `fastmcp` | ✅ synced | `main` |
| `edict` | ✅ synced | `main` |
| `microsoft-agent-framework` | ✅ synced | `main` |
| `swarms` | ✅ synced | `master` |
| `mission-control` | ✅ synced | `main` |
| `skills-mattpocock` | ✅ synced | `main` |
| `claude-mem` | ✅ synced | `main` |
| `oh-my-openagent` | ✅ synced | `dev` |
| `potpie` | ✅ synced | `main` |
| `cc-sdd` | ✅ synced | `main` |
| `gsd-2` | ✅ synced | `main` |
| `github-mcp-server` | ✅ synced | `main` |
| `my-claude-code-setup` | ✅ synced | `master` |
| `claude-token-efficient` | ⚠️ can't sync because there are diverging changes; use `--force` to overwrite the destination branch  | `main` |
| `playwright-mcp` | ✅ synced | `main` |
| `mcp-toolbox` | ✅ synced | `main` |
| `awesome-codex-subagents` | ✅ synced | `main` |
| `Awesome-Context-Engineering` | ✅ synced | `main` |
| `graphify` | ✅ synced | `v8` |
| `Understand-Anything` | ✅ synced | `main` |
| `awesome-openclaw-skills` | ✅ synced | `main` |
| `gsd-core` | ✅ synced | `next` |
| `agent-os` | ✅ synced | `main` |
| `ponytail` | ✅ synced | `main` |
| `agent-skills` | ✅ synced | `main` |
| `last30days-skill` | ✅ synced | `main` |
| `headroom` | ✅ synced | `main` |
| `Figma-Context-MCP` | ✅ synced | `main` |
| `agent-rules-books` | ✅ synced | `main` |
| `open-saas` | ✅ synced | `main` |
| `openclaw-mission-control` | ✅ synced | `master` |
| `awesome-claude-design` | ✅ synced | `main` |
| `mcp-inspector` | ✅ synced | `main` |
| `codebase-memory-mcp` | ✅ synced | `main` |
| `node-typescript-boilerplate` | ✅ synced | `main` |


## 2026-08-24T04:31:56Z

**Summary**: ✅ 101 synced · ⚠️ 5 failed · ❌ 0 skipped (total 106)

| Repo | Result | Branch |
|---|---|---|
| `skills` | ✅ synced | `main` |
| `superpowers` | ✅ synced | `main` |
| `andrej-karpathy-skills` | ✅ synced | `main` |
| `gstack` | ✅ synced | `main` |
| `everything-claude-code` | ✅ synced | `main` |
| `awesome-claude-code` | ⚠️ can't sync because there are diverging changes; use `--force` to overwrite the destination branch  | `main` |
| `awesome-claude-skills` | ✅ synced | `main` |
| `antigravity-awesome-skills` | ✅ synced | `main` |
| `claude-howto` | ✅ synced | `main` |
| `awesome-agent-skills` | ✅ synced | `main` |
| `awesome-claude-code-subagents` | ✅ synced | `main` |
| `ralph` | ✅ synced | `main` |
| `awesome-claude-skills-travisvn` | ✅ synced | `main` |
| `context-engineering-intro` | ✅ synced | `main` |
| `claude-code-best-practice` | ✅ synced | `main` |
| `how-claude-code-works` | ✅ synced | `main` |
| `pro-workflow` | ✅ synced | `main` |
| `claude-agents` | ✅ synced | `main` |
| `awesome-claude-code-toolkit` | ✅ synced | `main` |
| `claude-code-sub-agents` | ✅ synced | `main` |
| `claude-code-subagents` | ✅ synced | `main` |
| `awesome-claude-code-plugins` | ✅ synced | `main` |
| `awesome-claude-agents` | ✅ synced | `main` |
| `awesome-agent-skills-heilcheng` | ✅ synced | `main` |
| `awesome-claude-code-jqueryscript` | ✅ synced | `main` |
| `awesome-claude` | ✅ synced | `main` |
| `claude-code-hooks-mastery` | ✅ synced | `main` |
| `claude-code-infrastructure-showcase` | ✅ synced | `main` |
| `claude-code-showcase` | ✅ synced | `main` |
| `Continuous-Claude-v3` | ✅ synced | `main` |
| `buildwithclaude` | ✅ synced | `main` |
| `tdd-guard` | ✅ synced | `main` |
| `Claude-Code-Everything-You-Need-to-Know` | ✅ synced | `main` |
| `tweakcc` | ✅ synced | `main` |
| `oh-my-claudecode` | ✅ synced | `main` |
| `awesome-cursorrules` | ✅ synced | `main` |
| `awesome-copilot` | ⚠️ can't sync because there are diverging changes; use `--force` to overwrite the destination branch  | `main` |
| `awesome-copilot-agents` | ✅ synced | `main` |
| `awesome-cursorrules-zh` | ✅ synced | `master` |
| `myclaude` | ✅ synced | `master` |
| `overstory` | ✅ synced | `main` |
| `microsoft-skills` | ✅ synced | `main` |
| `spec-kit` | ✅ synced | `main` |
| `OpenSpec` | ✅ synced | `main` |
| `spec-workflow-mcp` | ✅ synced | `main` |
| `get-shit-done` | ✅ synced | `main` |
| `shotgun` | ✅ synced | `main` |
| `agents` | ✅ synced | `main` |
| `ruflo` | ⚠️ can't sync because there are diverging changes; use `--force` to overwrite the destination branch  | `main` |
| `open-multi-agent` | ✅ synced | `main` |
| `maestro-orchestrate` | ✅ synced | `main` |
| `flow-next` | ✅ synced | `main` |
| `claude-code-workflow` | ✅ synced | `main` |
| `context-engineering-kit` | ✅ synced | `master` |
| `mcp-servers-official` | ✅ synced | `main` |
| `awesome-mcp-servers` | ✅ synced | `main` |
| `Awesome-MCP-ZH` | ✅ synced | `main` |
| `awesome-remote-mcp-servers` | ✅ synced | `main` |
| `awesome-claude-plugins` | ✅ synced | `master` |
| `awesome-mcp-servers-appcypher` | ✅ synced | `main` |
| `agents.md` | ✅ synced | `main` |
| `design.md` | ✅ synced | `main` |
| `awesome-design-md` | ✅ synced | `main` |
| `Prompt-Engineering-Guide` | ✅ synced | `main` |
| `prompts.chat` | ✅ synced | `main` |
| `SwiftAgents` | ✅ synced | `main` |
| `rails_ai_agents` | ✅ synced | `main` |
| `awesome-openclaw-agents` | ✅ synced | `main` |
| `claude-code-templates` | ⚠️ can't sync because there are diverging changes; use `--force` to overwrite the destination branch  | `main` |
| `claude-code-system-prompts` | ✅ synced | `main` |
| `claude-code-ultimate-guide` | ✅ synced | `main` |
| `fastmcp` | ✅ synced | `main` |
| `edict` | ✅ synced | `main` |
| `microsoft-agent-framework` | ✅ synced | `main` |
| `swarms` | ✅ synced | `master` |
| `mission-control` | ✅ synced | `main` |
| `skills-mattpocock` | ✅ synced | `main` |
| `claude-mem` | ✅ synced | `main` |
| `oh-my-openagent` | ✅ synced | `dev` |
| `potpie` | ✅ synced | `main` |
| `cc-sdd` | ✅ synced | `main` |
| `gsd-2` | ✅ synced | `main` |
| `github-mcp-server` | ✅ synced | `main` |
| `my-claude-code-setup` | ✅ synced | `master` |
| `claude-token-efficient` | ⚠️ can't sync because there are diverging changes; use `--force` to overwrite the destination branch  | `main` |
| `playwright-mcp` | ✅ synced | `main` |
| `mcp-toolbox` | ✅ synced | `main` |
| `awesome-codex-subagents` | ✅ synced | `main` |
| `Awesome-Context-Engineering` | ✅ synced | `main` |
| `graphify` | ✅ synced | `v8` |
| `Understand-Anything` | ✅ synced | `main` |
| `awesome-openclaw-skills` | ✅ synced | `main` |
| `gsd-core` | ✅ synced | `next` |
| `agent-os` | ✅ synced | `main` |
| `ponytail` | ✅ synced | `main` |
| `agent-skills` | ✅ synced | `main` |
| `last30days-skill` | ✅ synced | `main` |
| `headroom` | ✅ synced | `main` |
| `Figma-Context-MCP` | ✅ synced | `main` |
| `agent-rules-books` | ✅ synced | `main` |
| `open-saas` | ✅ synced | `main` |
| `openclaw-mission-control` | ✅ synced | `master` |
| `awesome-claude-design` | ✅ synced | `main` |
| `mcp-inspector` | ✅ synced | `main` |
| `codebase-memory-mcp` | ✅ synced | `main` |
| `node-typescript-boilerplate` | ✅ synced | `main` |


## 2026-08-17T04:28:02Z

**Summary**: ✅ 101 synced · ⚠️ 5 failed · ❌ 0 skipped (total 106)

| Repo | Result | Branch |
|---|---|---|
| `skills` | ✅ synced | `main` |
| `superpowers` | ✅ synced | `main` |
| `andrej-karpathy-skills` | ✅ synced | `main` |
| `gstack` | ✅ synced | `main` |
| `everything-claude-code` | ✅ synced | `main` |
| `awesome-claude-code` | ⚠️ can't sync because there are diverging changes; use `--force` to overwrite the destination branch  | `main` |
| `awesome-claude-skills` | ✅ synced | `main` |
| `antigravity-awesome-skills` | ✅ synced | `main` |
| `claude-howto` | ✅ synced | `main` |
| `awesome-agent-skills` | ✅ synced | `main` |
| `awesome-claude-code-subagents` | ✅ synced | `main` |
| `ralph` | ✅ synced | `main` |
| `awesome-claude-skills-travisvn` | ✅ synced | `main` |
| `context-engineering-intro` | ✅ synced | `main` |
| `claude-code-best-practice` | ✅ synced | `main` |
| `how-claude-code-works` | ✅ synced | `main` |
| `pro-workflow` | ✅ synced | `main` |
| `claude-agents` | ✅ synced | `main` |
| `awesome-claude-code-toolkit` | ✅ synced | `main` |
| `claude-code-sub-agents` | ✅ synced | `main` |
| `claude-code-subagents` | ✅ synced | `main` |
| `awesome-claude-code-plugins` | ✅ synced | `main` |
| `awesome-claude-agents` | ✅ synced | `main` |
| `awesome-agent-skills-heilcheng` | ✅ synced | `main` |
| `awesome-claude-code-jqueryscript` | ✅ synced | `main` |
| `awesome-claude` | ✅ synced | `main` |
| `claude-code-hooks-mastery` | ✅ synced | `main` |
| `claude-code-infrastructure-showcase` | ✅ synced | `main` |
| `claude-code-showcase` | ✅ synced | `main` |
| `Continuous-Claude-v3` | ✅ synced | `main` |
| `buildwithclaude` | ✅ synced | `main` |
| `tdd-guard` | ✅ synced | `main` |
| `Claude-Code-Everything-You-Need-to-Know` | ✅ synced | `main` |
| `tweakcc` | ✅ synced | `main` |
| `oh-my-claudecode` | ✅ synced | `main` |
| `awesome-cursorrules` | ✅ synced | `main` |
| `awesome-copilot` | ⚠️ can't sync because there are diverging changes; use `--force` to overwrite the destination branch  | `main` |
| `awesome-copilot-agents` | ✅ synced | `main` |
| `awesome-cursorrules-zh` | ✅ synced | `master` |
| `myclaude` | ✅ synced | `master` |
| `overstory` | ✅ synced | `main` |
| `microsoft-skills` | ✅ synced | `main` |
| `spec-kit` | ✅ synced | `main` |
| `OpenSpec` | ✅ synced | `main` |
| `spec-workflow-mcp` | ✅ synced | `main` |
| `get-shit-done` | ✅ synced | `main` |
| `shotgun` | ✅ synced | `main` |
| `agents` | ✅ synced | `main` |
| `ruflo` | ⚠️ can't sync because there are diverging changes; use `--force` to overwrite the destination branch  | `main` |
| `open-multi-agent` | ✅ synced | `main` |
| `maestro-orchestrate` | ✅ synced | `main` |
| `flow-next` | ✅ synced | `main` |
| `claude-code-workflow` | ✅ synced | `main` |
| `context-engineering-kit` | ✅ synced | `master` |
| `mcp-servers-official` | ✅ synced | `main` |
| `awesome-mcp-servers` | ✅ synced | `main` |
| `Awesome-MCP-ZH` | ✅ synced | `main` |
| `awesome-remote-mcp-servers` | ✅ synced | `main` |
| `awesome-claude-plugins` | ✅ synced | `master` |
| `awesome-mcp-servers-appcypher` | ✅ synced | `main` |
| `agents.md` | ✅ synced | `main` |
| `design.md` | ✅ synced | `main` |
| `awesome-design-md` | ✅ synced | `main` |
| `Prompt-Engineering-Guide` | ✅ synced | `main` |
| `prompts.chat` | ✅ synced | `main` |
| `SwiftAgents` | ✅ synced | `main` |
| `rails_ai_agents` | ✅ synced | `main` |
| `awesome-openclaw-agents` | ✅ synced | `main` |
| `claude-code-templates` | ⚠️ can't sync because there are diverging changes; use `--force` to overwrite the destination branch  | `main` |
| `claude-code-system-prompts` | ✅ synced | `main` |
| `claude-code-ultimate-guide` | ✅ synced | `main` |
| `fastmcp` | ✅ synced | `main` |
| `edict` | ✅ synced | `main` |
| `microsoft-agent-framework` | ✅ synced | `main` |
| `swarms` | ✅ synced | `master` |
| `mission-control` | ✅ synced | `main` |
| `skills-mattpocock` | ✅ synced | `main` |
| `claude-mem` | ✅ synced | `main` |
| `oh-my-openagent` | ✅ synced | `dev` |
| `potpie` | ✅ synced | `main` |
| `cc-sdd` | ✅ synced | `main` |
| `gsd-2` | ✅ synced | `main` |
| `github-mcp-server` | ✅ synced | `main` |
| `my-claude-code-setup` | ✅ synced | `master` |
| `claude-token-efficient` | ⚠️ can't sync because there are diverging changes; use `--force` to overwrite the destination branch  | `main` |
| `playwright-mcp` | ✅ synced | `main` |
| `mcp-toolbox` | ✅ synced | `main` |
| `awesome-codex-subagents` | ✅ synced | `main` |
| `Awesome-Context-Engineering` | ✅ synced | `main` |
| `graphify` | ✅ synced | `v8` |
| `Understand-Anything` | ✅ synced | `main` |
| `awesome-openclaw-skills` | ✅ synced | `main` |
| `gsd-core` | ✅ synced | `next` |
| `agent-os` | ✅ synced | `main` |
| `ponytail` | ✅ synced | `main` |
| `agent-skills` | ✅ synced | `main` |
| `last30days-skill` | ✅ synced | `main` |
| `headroom` | ✅ synced | `main` |
| `Figma-Context-MCP` | ✅ synced | `main` |
| `agent-rules-books` | ✅ synced | `main` |
| `open-saas` | ✅ synced | `main` |
| `openclaw-mission-control` | ✅ synced | `master` |
| `awesome-claude-design` | ✅ synced | `main` |
| `mcp-inspector` | ✅ synced | `main` |
| `codebase-memory-mcp` | ✅ synced | `main` |
| `node-typescript-boilerplate` | ✅ synced | `main` |


## 2026-08-10T05:12:32Z

**Summary**: ✅ 101 synced · ⚠️ 5 failed · ❌ 0 skipped (total 106)

| Repo | Result | Branch |
|---|---|---|
| `skills` | ✅ synced | `main` |
| `superpowers` | ✅ synced | `main` |
| `andrej-karpathy-skills` | ✅ synced | `main` |
| `gstack` | ✅ synced | `main` |
| `everything-claude-code` | ✅ synced | `main` |
| `awesome-claude-code` | ⚠️ can't sync because there are diverging changes; use `--force` to overwrite the destination branch  | `main` |
| `awesome-claude-skills` | ✅ synced | `main` |
| `antigravity-awesome-skills` | ✅ synced | `main` |
| `claude-howto` | ✅ synced | `main` |
| `awesome-agent-skills` | ✅ synced | `main` |
| `awesome-claude-code-subagents` | ✅ synced | `main` |
| `ralph` | ✅ synced | `main` |
| `awesome-claude-skills-travisvn` | ✅ synced | `main` |
| `context-engineering-intro` | ✅ synced | `main` |
| `claude-code-best-practice` | ✅ synced | `main` |
| `how-claude-code-works` | ✅ synced | `main` |
| `pro-workflow` | ✅ synced | `main` |
| `claude-agents` | ✅ synced | `main` |
| `awesome-claude-code-toolkit` | ✅ synced | `main` |
| `claude-code-sub-agents` | ✅ synced | `main` |
| `claude-code-subagents` | ✅ synced | `main` |
| `awesome-claude-code-plugins` | ✅ synced | `main` |
| `awesome-claude-agents` | ✅ synced | `main` |
| `awesome-agent-skills-heilcheng` | ✅ synced | `main` |
| `awesome-claude-code-jqueryscript` | ✅ synced | `main` |
| `awesome-claude` | ✅ synced | `main` |
| `claude-code-hooks-mastery` | ✅ synced | `main` |
| `claude-code-infrastructure-showcase` | ✅ synced | `main` |
| `claude-code-showcase` | ✅ synced | `main` |
| `Continuous-Claude-v3` | ✅ synced | `main` |
| `buildwithclaude` | ✅ synced | `main` |
| `tdd-guard` | ✅ synced | `main` |
| `Claude-Code-Everything-You-Need-to-Know` | ✅ synced | `main` |
| `tweakcc` | ✅ synced | `main` |
| `oh-my-claudecode` | ✅ synced | `main` |
| `awesome-cursorrules` | ✅ synced | `main` |
| `awesome-copilot` | ⚠️ can't sync because there are diverging changes; use `--force` to overwrite the destination branch  | `main` |
| `awesome-copilot-agents` | ✅ synced | `main` |
| `awesome-cursorrules-zh` | ✅ synced | `master` |
| `myclaude` | ✅ synced | `master` |
| `overstory` | ✅ synced | `main` |
| `microsoft-skills` | ✅ synced | `main` |
| `spec-kit` | ✅ synced | `main` |
| `OpenSpec` | ✅ synced | `main` |
| `spec-workflow-mcp` | ✅ synced | `main` |
| `get-shit-done` | ✅ synced | `main` |
| `shotgun` | ✅ synced | `main` |
| `agents` | ✅ synced | `main` |
| `ruflo` | ⚠️ can't sync because there are diverging changes; use `--force` to overwrite the destination branch  | `main` |
| `open-multi-agent` | ✅ synced | `main` |
| `maestro-orchestrate` | ✅ synced | `main` |
| `flow-next` | ✅ synced | `main` |
| `claude-code-workflow` | ✅ synced | `main` |
| `context-engineering-kit` | ✅ synced | `master` |
| `mcp-servers-official` | ✅ synced | `main` |
| `awesome-mcp-servers` | ✅ synced | `main` |
| `Awesome-MCP-ZH` | ✅ synced | `main` |
| `awesome-remote-mcp-servers` | ✅ synced | `main` |
| `awesome-claude-plugins` | ✅ synced | `master` |
| `awesome-mcp-servers-appcypher` | ✅ synced | `main` |
| `agents.md` | ✅ synced | `main` |
| `design.md` | ✅ synced | `main` |
| `awesome-design-md` | ✅ synced | `main` |
| `Prompt-Engineering-Guide` | ✅ synced | `main` |
| `prompts.chat` | ✅ synced | `main` |
| `SwiftAgents` | ✅ synced | `main` |
| `rails_ai_agents` | ✅ synced | `main` |
| `awesome-openclaw-agents` | ✅ synced | `main` |
| `claude-code-templates` | ⚠️ can't sync because there are diverging changes; use `--force` to overwrite the destination branch  | `main` |
| `claude-code-system-prompts` | ✅ synced | `main` |
| `claude-code-ultimate-guide` | ✅ synced | `main` |
| `fastmcp` | ✅ synced | `main` |
| `edict` | ✅ synced | `main` |
| `microsoft-agent-framework` | ✅ synced | `main` |
| `swarms` | ✅ synced | `master` |
| `mission-control` | ✅ synced | `main` |
| `skills-mattpocock` | ✅ synced | `main` |
| `claude-mem` | ✅ synced | `main` |
| `oh-my-openagent` | ✅ synced | `dev` |
| `potpie` | ✅ synced | `main` |
| `cc-sdd` | ✅ synced | `main` |
| `gsd-2` | ✅ synced | `main` |
| `github-mcp-server` | ✅ synced | `main` |
| `my-claude-code-setup` | ✅ synced | `master` |
| `claude-token-efficient` | ⚠️ can't sync because there are diverging changes; use `--force` to overwrite the destination branch  | `main` |
| `playwright-mcp` | ✅ synced | `main` |
| `mcp-toolbox` | ✅ synced | `main` |
| `awesome-codex-subagents` | ✅ synced | `main` |
| `Awesome-Context-Engineering` | ✅ synced | `main` |
| `graphify` | ✅ synced | `v8` |
| `Understand-Anything` | ✅ synced | `main` |
| `awesome-openclaw-skills` | ✅ synced | `main` |
| `gsd-core` | ✅ synced | `next` |
| `agent-os` | ✅ synced | `main` |
| `ponytail` | ✅ synced | `main` |
| `agent-skills` | ✅ synced | `main` |
| `last30days-skill` | ✅ synced | `main` |
| `headroom` | ✅ synced | `main` |
| `Figma-Context-MCP` | ✅ synced | `main` |
| `agent-rules-books` | ✅ synced | `main` |
| `open-saas` | ✅ synced | `main` |
| `openclaw-mission-control` | ✅ synced | `master` |
| `awesome-claude-design` | ✅ synced | `main` |
| `mcp-inspector` | ✅ synced | `main` |
| `codebase-memory-mcp` | ✅ synced | `main` |
| `node-typescript-boilerplate` | ✅ synced | `main` |


## 2026-08-03T06:48:49Z

**Summary**: ✅ 101 synced · ⚠️ 5 failed · ❌ 0 skipped (total 106)

| Repo | Result | Branch |
|---|---|---|
| `skills` | ✅ synced | `main` |
| `superpowers` | ✅ synced | `main` |
| `andrej-karpathy-skills` | ✅ synced | `main` |
| `gstack` | ✅ synced | `main` |
| `everything-claude-code` | ✅ synced | `main` |
| `awesome-claude-code` | ⚠️ can't sync because there are diverging changes; use `--force` to overwrite the destination branch  | `main` |
| `awesome-claude-skills` | ✅ synced | `main` |
| `antigravity-awesome-skills` | ✅ synced | `main` |
| `claude-howto` | ✅ synced | `main` |
| `awesome-agent-skills` | ✅ synced | `main` |
| `awesome-claude-code-subagents` | ✅ synced | `main` |
| `ralph` | ✅ synced | `main` |
| `awesome-claude-skills-travisvn` | ✅ synced | `main` |
| `context-engineering-intro` | ✅ synced | `main` |
| `claude-code-best-practice` | ✅ synced | `main` |
| `how-claude-code-works` | ✅ synced | `main` |
| `pro-workflow` | ✅ synced | `main` |
| `claude-agents` | ✅ synced | `main` |
| `awesome-claude-code-toolkit` | ✅ synced | `main` |
| `claude-code-sub-agents` | ✅ synced | `main` |
| `claude-code-subagents` | ✅ synced | `main` |
| `awesome-claude-code-plugins` | ✅ synced | `main` |
| `awesome-claude-agents` | ✅ synced | `main` |
| `awesome-agent-skills-heilcheng` | ✅ synced | `main` |
| `awesome-claude-code-jqueryscript` | ✅ synced | `main` |
| `awesome-claude` | ✅ synced | `main` |
| `claude-code-hooks-mastery` | ✅ synced | `main` |
| `claude-code-infrastructure-showcase` | ✅ synced | `main` |
| `claude-code-showcase` | ✅ synced | `main` |
| `Continuous-Claude-v3` | ✅ synced | `main` |
| `buildwithclaude` | ✅ synced | `main` |
| `tdd-guard` | ✅ synced | `main` |
| `Claude-Code-Everything-You-Need-to-Know` | ✅ synced | `main` |
| `tweakcc` | ✅ synced | `main` |
| `oh-my-claudecode` | ✅ synced | `main` |
| `awesome-cursorrules` | ✅ synced | `main` |
| `awesome-copilot` | ⚠️ can't sync because there are diverging changes; use `--force` to overwrite the destination branch  | `main` |
| `awesome-copilot-agents` | ✅ synced | `main` |
| `awesome-cursorrules-zh` | ✅ synced | `master` |
| `myclaude` | ✅ synced | `master` |
| `overstory` | ✅ synced | `main` |
| `microsoft-skills` | ✅ synced | `main` |
| `spec-kit` | ✅ synced | `main` |
| `OpenSpec` | ✅ synced | `main` |
| `spec-workflow-mcp` | ✅ synced | `main` |
| `get-shit-done` | ✅ synced | `main` |
| `shotgun` | ✅ synced | `main` |
| `agents` | ✅ synced | `main` |
| `ruflo` | ⚠️ can't sync because there are diverging changes; use `--force` to overwrite the destination branch  | `main` |
| `open-multi-agent` | ✅ synced | `main` |
| `maestro-orchestrate` | ✅ synced | `main` |
| `flow-next` | ✅ synced | `main` |
| `claude-code-workflow` | ✅ synced | `main` |
| `context-engineering-kit` | ✅ synced | `master` |
| `mcp-servers-official` | ✅ synced | `main` |
| `awesome-mcp-servers` | ✅ synced | `main` |
| `Awesome-MCP-ZH` | ✅ synced | `main` |
| `awesome-remote-mcp-servers` | ✅ synced | `main` |
| `awesome-claude-plugins` | ✅ synced | `master` |
| `awesome-mcp-servers-appcypher` | ✅ synced | `main` |
| `agents.md` | ✅ synced | `main` |
| `design.md` | ✅ synced | `main` |
| `awesome-design-md` | ✅ synced | `main` |
| `Prompt-Engineering-Guide` | ✅ synced | `main` |
| `prompts.chat` | ✅ synced | `main` |
| `SwiftAgents` | ✅ synced | `main` |
| `rails_ai_agents` | ✅ synced | `main` |
| `awesome-openclaw-agents` | ✅ synced | `main` |
| `claude-code-templates` | ⚠️ can't sync because there are diverging changes; use `--force` to overwrite the destination branch  | `main` |
| `claude-code-system-prompts` | ✅ synced | `main` |
| `claude-code-ultimate-guide` | ✅ synced | `main` |
| `fastmcp` | ✅ synced | `main` |
| `edict` | ✅ synced | `main` |
| `microsoft-agent-framework` | ✅ synced | `main` |
| `swarms` | ✅ synced | `master` |
| `mission-control` | ✅ synced | `main` |
| `skills-mattpocock` | ✅ synced | `main` |
| `claude-mem` | ✅ synced | `main` |
| `oh-my-openagent` | ✅ synced | `dev` |
| `potpie` | ✅ synced | `main` |
| `cc-sdd` | ✅ synced | `main` |
| `gsd-2` | ✅ synced | `main` |
| `github-mcp-server` | ✅ synced | `main` |
| `my-claude-code-setup` | ✅ synced | `master` |
| `claude-token-efficient` | ⚠️ can't sync because there are diverging changes; use `--force` to overwrite the destination branch  | `main` |
| `playwright-mcp` | ✅ synced | `main` |
| `mcp-toolbox` | ✅ synced | `main` |
| `awesome-codex-subagents` | ✅ synced | `main` |
| `Awesome-Context-Engineering` | ✅ synced | `main` |
| `graphify` | ✅ synced | `v8` |
| `Understand-Anything` | ✅ synced | `main` |
| `awesome-openclaw-skills` | ✅ synced | `main` |
| `gsd-core` | ✅ synced | `next` |
| `agent-os` | ✅ synced | `main` |
| `ponytail` | ✅ synced | `main` |
| `agent-skills` | ✅ synced | `main` |
| `last30days-skill` | ✅ synced | `main` |
| `headroom` | ✅ synced | `main` |
| `Figma-Context-MCP` | ✅ synced | `main` |
| `agent-rules-books` | ✅ synced | `main` |
| `open-saas` | ✅ synced | `main` |
| `openclaw-mission-control` | ✅ synced | `master` |
| `awesome-claude-design` | ✅ synced | `main` |
| `mcp-inspector` | ✅ synced | `main` |
| `codebase-memory-mcp` | ✅ synced | `main` |
| `node-typescript-boilerplate` | ✅ synced | `main` |


## 2026-07-27T06:52:03Z

**Summary**: ✅ 101 synced · ⚠️ 5 failed · ❌ 0 skipped (total 106)

| Repo | Result | Branch |
|---|---|---|
| `skills` | ✅ synced | `main` |
| `superpowers` | ✅ synced | `main` |
| `andrej-karpathy-skills` | ✅ synced | `main` |
| `gstack` | ✅ synced | `main` |
| `everything-claude-code` | ✅ synced | `main` |
| `awesome-claude-code` | ⚠️ can't sync because there are diverging changes; use `--force` to overwrite the destination branch  | `main` |
| `awesome-claude-skills` | ✅ synced | `main` |
| `antigravity-awesome-skills` | ✅ synced | `main` |
| `claude-howto` | ✅ synced | `main` |
| `awesome-agent-skills` | ✅ synced | `main` |
| `awesome-claude-code-subagents` | ✅ synced | `main` |
| `ralph` | ✅ synced | `main` |
| `awesome-claude-skills-travisvn` | ✅ synced | `main` |
| `context-engineering-intro` | ✅ synced | `main` |
| `claude-code-best-practice` | ✅ synced | `main` |
| `how-claude-code-works` | ✅ synced | `main` |
| `pro-workflow` | ✅ synced | `main` |
| `claude-agents` | ✅ synced | `main` |
| `awesome-claude-code-toolkit` | ✅ synced | `main` |
| `claude-code-sub-agents` | ✅ synced | `main` |
| `claude-code-subagents` | ✅ synced | `main` |
| `awesome-claude-code-plugins` | ✅ synced | `main` |
| `awesome-claude-agents` | ✅ synced | `main` |
| `awesome-agent-skills-heilcheng` | ✅ synced | `main` |
| `awesome-claude-code-jqueryscript` | ✅ synced | `main` |
| `awesome-claude` | ✅ synced | `main` |
| `claude-code-hooks-mastery` | ✅ synced | `main` |
| `claude-code-infrastructure-showcase` | ✅ synced | `main` |
| `claude-code-showcase` | ✅ synced | `main` |
| `Continuous-Claude-v3` | ✅ synced | `main` |
| `buildwithclaude` | ✅ synced | `main` |
| `tdd-guard` | ✅ synced | `main` |
| `Claude-Code-Everything-You-Need-to-Know` | ✅ synced | `main` |
| `tweakcc` | ✅ synced | `main` |
| `oh-my-claudecode` | ✅ synced | `main` |
| `awesome-cursorrules` | ✅ synced | `main` |
| `awesome-copilot` | ⚠️ can't sync because there are diverging changes; use `--force` to overwrite the destination branch  | `main` |
| `awesome-copilot-agents` | ✅ synced | `main` |
| `awesome-cursorrules-zh` | ✅ synced | `master` |
| `myclaude` | ✅ synced | `master` |
| `overstory` | ✅ synced | `main` |
| `microsoft-skills` | ✅ synced | `main` |
| `spec-kit` | ✅ synced | `main` |
| `OpenSpec` | ✅ synced | `main` |
| `spec-workflow-mcp` | ✅ synced | `main` |
| `get-shit-done` | ✅ synced | `main` |
| `shotgun` | ✅ synced | `main` |
| `agents` | ✅ synced | `main` |
| `ruflo` | ⚠️ can't sync because there are diverging changes; use `--force` to overwrite the destination branch  | `main` |
| `open-multi-agent` | ✅ synced | `main` |
| `maestro-orchestrate` | ✅ synced | `main` |
| `flow-next` | ✅ synced | `main` |
| `claude-code-workflow` | ✅ synced | `main` |
| `context-engineering-kit` | ✅ synced | `master` |
| `mcp-servers-official` | ✅ synced | `main` |
| `awesome-mcp-servers` | ✅ synced | `main` |
| `Awesome-MCP-ZH` | ✅ synced | `main` |
| `awesome-remote-mcp-servers` | ✅ synced | `main` |
| `awesome-claude-plugins` | ✅ synced | `master` |
| `awesome-mcp-servers-appcypher` | ✅ synced | `main` |
| `agents.md` | ✅ synced | `main` |
| `design.md` | ✅ synced | `main` |
| `awesome-design-md` | ✅ synced | `main` |
| `Prompt-Engineering-Guide` | ✅ synced | `main` |
| `prompts.chat` | ✅ synced | `main` |
| `SwiftAgents` | ✅ synced | `main` |
| `rails_ai_agents` | ✅ synced | `main` |
| `awesome-openclaw-agents` | ✅ synced | `main` |
| `claude-code-templates` | ⚠️ can't sync because there are diverging changes; use `--force` to overwrite the destination branch  | `main` |
| `claude-code-system-prompts` | ✅ synced | `main` |
| `claude-code-ultimate-guide` | ✅ synced | `main` |
| `fastmcp` | ✅ synced | `main` |
| `edict` | ✅ synced | `main` |
| `microsoft-agent-framework` | ✅ synced | `main` |
| `swarms` | ✅ synced | `master` |
| `mission-control` | ✅ synced | `main` |
| `skills-mattpocock` | ✅ synced | `main` |
| `claude-mem` | ✅ synced | `main` |
| `oh-my-openagent` | ✅ synced | `dev` |
| `potpie` | ✅ synced | `main` |
| `cc-sdd` | ✅ synced | `main` |
| `gsd-2` | ✅ synced | `main` |
| `github-mcp-server` | ✅ synced | `main` |
| `my-claude-code-setup` | ✅ synced | `master` |
| `claude-token-efficient` | ⚠️ can't sync because there are diverging changes; use `--force` to overwrite the destination branch  | `main` |
| `playwright-mcp` | ✅ synced | `main` |
| `mcp-toolbox` | ✅ synced | `main` |
| `awesome-codex-subagents` | ✅ synced | `main` |
| `Awesome-Context-Engineering` | ✅ synced | `main` |
| `graphify` | ✅ synced | `v8` |
| `Understand-Anything` | ✅ synced | `main` |
| `awesome-openclaw-skills` | ✅ synced | `main` |
| `gsd-core` | ✅ synced | `next` |
| `agent-os` | ✅ synced | `main` |
| `ponytail` | ✅ synced | `main` |
| `agent-skills` | ✅ synced | `main` |
| `last30days-skill` | ✅ synced | `main` |
| `headroom` | ✅ synced | `main` |
| `Figma-Context-MCP` | ✅ synced | `main` |
| `agent-rules-books` | ✅ synced | `main` |
| `open-saas` | ✅ synced | `main` |
| `openclaw-mission-control` | ✅ synced | `master` |
| `awesome-claude-design` | ✅ synced | `main` |
| `mcp-inspector` | ✅ synced | `main` |
| `codebase-memory-mcp` | ✅ synced | `main` |
| `node-typescript-boilerplate` | ✅ synced | `main` |


## 2026-07-20T06:35:45Z

**Summary**: ✅ 89 synced · ⚠️ 5 failed · ❌ 0 skipped (total 94)

| Repo | Result | Branch |
|---|---|---|
| `skills` | ✅ synced | `main` |
| `superpowers` | ✅ synced | `main` |
| `andrej-karpathy-skills` | ✅ synced | `main` |
| `gstack` | ✅ synced | `main` |
| `everything-claude-code` | ✅ synced | `main` |
| `awesome-claude-code` | ⚠️ can't sync because there are diverging changes; use `--force` to overwrite the destination branch  | `main` |
| `awesome-claude-skills` | ✅ synced | `main` |
| `antigravity-awesome-skills` | ✅ synced | `main` |
| `claude-howto` | ✅ synced | `main` |
| `awesome-agent-skills` | ✅ synced | `main` |
| `awesome-claude-code-subagents` | ✅ synced | `main` |
| `ralph` | ✅ synced | `main` |
| `awesome-claude-skills-travisvn` | ✅ synced | `main` |
| `context-engineering-intro` | ✅ synced | `main` |
| `claude-code-best-practice` | ✅ synced | `main` |
| `how-claude-code-works` | ✅ synced | `main` |
| `pro-workflow` | ✅ synced | `main` |
| `claude-agents` | ✅ synced | `main` |
| `awesome-claude-code-toolkit` | ✅ synced | `main` |
| `claude-code-sub-agents` | ✅ synced | `main` |
| `claude-code-subagents` | ✅ synced | `main` |
| `awesome-claude-code-plugins` | ✅ synced | `main` |
| `awesome-claude-agents` | ✅ synced | `main` |
| `awesome-agent-skills-heilcheng` | ✅ synced | `main` |
| `awesome-claude-code-jqueryscript` | ✅ synced | `main` |
| `awesome-claude` | ✅ synced | `main` |
| `claude-code-hooks-mastery` | ✅ synced | `main` |
| `claude-code-infrastructure-showcase` | ✅ synced | `main` |
| `claude-code-showcase` | ✅ synced | `main` |
| `Continuous-Claude-v3` | ✅ synced | `main` |
| `buildwithclaude` | ✅ synced | `main` |
| `tdd-guard` | ✅ synced | `main` |
| `Claude-Code-Everything-You-Need-to-Know` | ✅ synced | `main` |
| `tweakcc` | ✅ synced | `main` |
| `oh-my-claudecode` | ✅ synced | `main` |
| `awesome-cursorrules` | ✅ synced | `main` |
| `awesome-copilot` | ⚠️ can't sync because there are diverging changes; use `--force` to overwrite the destination branch  | `main` |
| `awesome-copilot-agents` | ✅ synced | `main` |
| `awesome-cursorrules-zh` | ✅ synced | `master` |
| `myclaude` | ✅ synced | `master` |
| `overstory` | ✅ synced | `main` |
| `microsoft-skills` | ✅ synced | `main` |
| `spec-kit` | ✅ synced | `main` |
| `OpenSpec` | ✅ synced | `main` |
| `spec-workflow-mcp` | ✅ synced | `main` |
| `get-shit-done` | ✅ synced | `main` |
| `shotgun` | ✅ synced | `main` |
| `agents` | ✅ synced | `main` |
| `ruflo` | ⚠️ can't sync because there are diverging changes; use `--force` to overwrite the destination branch  | `main` |
| `open-multi-agent` | ✅ synced | `main` |
| `maestro-orchestrate` | ✅ synced | `main` |
| `flow-next` | ✅ synced | `main` |
| `claude-code-workflow` | ✅ synced | `main` |
| `context-engineering-kit` | ✅ synced | `master` |
| `mcp-servers-official` | ✅ synced | `main` |
| `awesome-mcp-servers` | ✅ synced | `main` |
| `Awesome-MCP-ZH` | ✅ synced | `main` |
| `awesome-remote-mcp-servers` | ✅ synced | `main` |
| `awesome-claude-plugins` | ✅ synced | `master` |
| `awesome-mcp-servers-appcypher` | ✅ synced | `main` |
| `agents.md` | ✅ synced | `main` |
| `design.md` | ✅ synced | `main` |
| `awesome-design-md` | ✅ synced | `main` |
| `Prompt-Engineering-Guide` | ✅ synced | `main` |
| `prompts.chat` | ✅ synced | `main` |
| `SwiftAgents` | ✅ synced | `main` |
| `rails_ai_agents` | ✅ synced | `main` |
| `awesome-openclaw-agents` | ✅ synced | `main` |
| `claude-code-templates` | ⚠️ can't sync because there are diverging changes; use `--force` to overwrite the destination branch  | `main` |
| `claude-code-system-prompts` | ✅ synced | `main` |
| `claude-code-ultimate-guide` | ✅ synced | `main` |
| `fastmcp` | ✅ synced | `main` |
| `edict` | ✅ synced | `main` |
| `microsoft-agent-framework` | ✅ synced | `main` |
| `swarms` | ✅ synced | `master` |
| `mission-control` | ✅ synced | `main` |
| `skills-mattpocock` | ✅ synced | `main` |
| `claude-mem` | ✅ synced | `main` |
| `oh-my-openagent` | ✅ synced | `dev` |
| `potpie` | ✅ synced | `main` |
| `cc-sdd` | ✅ synced | `main` |
| `gsd-2` | ✅ synced | `main` |
| `github-mcp-server` | ✅ synced | `main` |
| `my-claude-code-setup` | ✅ synced | `master` |
| `claude-token-efficient` | ⚠️ can't sync because there are diverging changes; use `--force` to overwrite the destination branch  | `main` |
| `playwright-mcp` | ✅ synced | `main` |
| `mcp-toolbox` | ✅ synced | `main` |
| `awesome-codex-subagents` | ✅ synced | `main` |
| `Awesome-Context-Engineering` | ✅ synced | `main` |
| `graphify` | ✅ synced | `v8` |
| `Understand-Anything` | ✅ synced | `main` |
| `awesome-openclaw-skills` | ✅ synced | `main` |
| `gsd-core` | ✅ synced | `next` |
| `agent-os` | ✅ synced | `main` |


## 2026-07-13T06:41:29Z

**Summary**: ✅ 89 synced · ⚠️ 5 failed · ❌ 0 skipped (total 94)

| Repo | Result | Branch |
|---|---|---|
| `skills` | ✅ synced | `main` |
| `superpowers` | ✅ synced | `main` |
| `andrej-karpathy-skills` | ✅ synced | `main` |
| `gstack` | ✅ synced | `main` |
| `everything-claude-code` | ✅ synced | `main` |
| `awesome-claude-code` | ⚠️ can't sync because there are diverging changes; use `--force` to overwrite the destination branch  | `main` |
| `awesome-claude-skills` | ✅ synced | `main` |
| `antigravity-awesome-skills` | ✅ synced | `main` |
| `claude-howto` | ✅ synced | `main` |
| `awesome-agent-skills` | ✅ synced | `main` |
| `awesome-claude-code-subagents` | ✅ synced | `main` |
| `ralph` | ✅ synced | `main` |
| `awesome-claude-skills-travisvn` | ✅ synced | `main` |
| `context-engineering-intro` | ✅ synced | `main` |
| `claude-code-best-practice` | ✅ synced | `main` |
| `how-claude-code-works` | ✅ synced | `main` |
| `pro-workflow` | ✅ synced | `main` |
| `claude-agents` | ✅ synced | `main` |
| `awesome-claude-code-toolkit` | ✅ synced | `main` |
| `claude-code-sub-agents` | ✅ synced | `main` |
| `claude-code-subagents` | ✅ synced | `main` |
| `awesome-claude-code-plugins` | ✅ synced | `main` |
| `awesome-claude-agents` | ✅ synced | `main` |
| `awesome-agent-skills-heilcheng` | ✅ synced | `main` |
| `awesome-claude-code-jqueryscript` | ✅ synced | `main` |
| `awesome-claude` | ✅ synced | `main` |
| `claude-code-hooks-mastery` | ✅ synced | `main` |
| `claude-code-infrastructure-showcase` | ✅ synced | `main` |
| `claude-code-showcase` | ✅ synced | `main` |
| `Continuous-Claude-v3` | ✅ synced | `main` |
| `buildwithclaude` | ✅ synced | `main` |
| `tdd-guard` | ✅ synced | `main` |
| `Claude-Code-Everything-You-Need-to-Know` | ✅ synced | `main` |
| `tweakcc` | ✅ synced | `main` |
| `oh-my-claudecode` | ✅ synced | `main` |
| `awesome-cursorrules` | ✅ synced | `main` |
| `awesome-copilot` | ⚠️ can't sync because there are diverging changes; use `--force` to overwrite the destination branch  | `main` |
| `awesome-copilot-agents` | ✅ synced | `main` |
| `awesome-cursorrules-zh` | ✅ synced | `master` |
| `myclaude` | ✅ synced | `master` |
| `overstory` | ✅ synced | `main` |
| `microsoft-skills` | ✅ synced | `main` |
| `spec-kit` | ✅ synced | `main` |
| `OpenSpec` | ✅ synced | `main` |
| `spec-workflow-mcp` | ✅ synced | `main` |
| `get-shit-done` | ✅ synced | `main` |
| `shotgun` | ✅ synced | `main` |
| `agents` | ✅ synced | `main` |
| `ruflo` | ⚠️ can't sync because there are diverging changes; use `--force` to overwrite the destination branch  | `main` |
| `open-multi-agent` | ✅ synced | `main` |
| `maestro-orchestrate` | ✅ synced | `main` |
| `flow-next` | ✅ synced | `main` |
| `claude-code-workflow` | ✅ synced | `main` |
| `context-engineering-kit` | ✅ synced | `master` |
| `mcp-servers-official` | ✅ synced | `main` |
| `awesome-mcp-servers` | ✅ synced | `main` |
| `Awesome-MCP-ZH` | ✅ synced | `main` |
| `awesome-remote-mcp-servers` | ✅ synced | `main` |
| `awesome-claude-plugins` | ✅ synced | `master` |
| `awesome-mcp-servers-appcypher` | ✅ synced | `main` |
| `agents.md` | ✅ synced | `main` |
| `design.md` | ✅ synced | `main` |
| `awesome-design-md` | ✅ synced | `main` |
| `Prompt-Engineering-Guide` | ✅ synced | `main` |
| `prompts.chat` | ✅ synced | `main` |
| `SwiftAgents` | ✅ synced | `main` |
| `rails_ai_agents` | ✅ synced | `main` |
| `awesome-openclaw-agents` | ✅ synced | `main` |
| `claude-code-templates` | ⚠️ can't sync because there are diverging changes; use `--force` to overwrite the destination branch  | `main` |
| `claude-code-system-prompts` | ✅ synced | `main` |
| `claude-code-ultimate-guide` | ✅ synced | `main` |
| `fastmcp` | ✅ synced | `main` |
| `edict` | ✅ synced | `main` |
| `microsoft-agent-framework` | ✅ synced | `main` |
| `swarms` | ✅ synced | `master` |
| `mission-control` | ✅ synced | `main` |
| `skills-mattpocock` | ✅ synced | `main` |
| `claude-mem` | ✅ synced | `main` |
| `oh-my-openagent` | ✅ synced | `dev` |
| `potpie` | ✅ synced | `main` |
| `cc-sdd` | ✅ synced | `main` |
| `gsd-2` | ✅ synced | `main` |
| `github-mcp-server` | ✅ synced | `main` |
| `my-claude-code-setup` | ✅ synced | `master` |
| `claude-token-efficient` | ⚠️ can't sync because there are diverging changes; use `--force` to overwrite the destination branch  | `main` |
| `playwright-mcp` | ✅ synced | `main` |
| `mcp-toolbox` | ✅ synced | `main` |
| `awesome-codex-subagents` | ✅ synced | `main` |
| `Awesome-Context-Engineering` | ✅ synced | `main` |
| `graphify` | ✅ synced | `v8` |
| `Understand-Anything` | ✅ synced | `main` |
| `awesome-openclaw-skills` | ✅ synced | `main` |
| `gsd-core` | ✅ synced | `next` |
| `agent-os` | ✅ synced | `main` |


## 2026-07-06T08:06:37Z

**Summary**: ✅ 90 synced · ⚠️ 4 failed · ❌ 0 skipped (total 94)

| Repo | Result | Branch |
|---|---|---|
| `skills` | ✅ synced | `main` |
| `superpowers` | ✅ synced | `main` |
| `andrej-karpathy-skills` | ✅ synced | `main` |
| `gstack` | ✅ synced | `main` |
| `everything-claude-code` | ✅ synced | `main` |
| `awesome-claude-code` | ✅ synced | `main` |
| `awesome-claude-skills` | ✅ synced | `main` |
| `antigravity-awesome-skills` | ✅ synced | `main` |
| `claude-howto` | ✅ synced | `main` |
| `awesome-agent-skills` | ✅ synced | `main` |
| `awesome-claude-code-subagents` | ✅ synced | `main` |
| `ralph` | ✅ synced | `main` |
| `awesome-claude-skills-travisvn` | ✅ synced | `main` |
| `context-engineering-intro` | ✅ synced | `main` |
| `claude-code-best-practice` | ✅ synced | `main` |
| `how-claude-code-works` | ✅ synced | `main` |
| `pro-workflow` | ✅ synced | `main` |
| `claude-agents` | ✅ synced | `main` |
| `awesome-claude-code-toolkit` | ✅ synced | `main` |
| `claude-code-sub-agents` | ✅ synced | `main` |
| `claude-code-subagents` | ✅ synced | `main` |
| `awesome-claude-code-plugins` | ✅ synced | `main` |
| `awesome-claude-agents` | ✅ synced | `main` |
| `awesome-agent-skills-heilcheng` | ✅ synced | `main` |
| `awesome-claude-code-jqueryscript` | ✅ synced | `main` |
| `awesome-claude` | ✅ synced | `main` |
| `claude-code-hooks-mastery` | ✅ synced | `main` |
| `claude-code-infrastructure-showcase` | ✅ synced | `main` |
| `claude-code-showcase` | ✅ synced | `main` |
| `Continuous-Claude-v3` | ✅ synced | `main` |
| `buildwithclaude` | ✅ synced | `main` |
| `tdd-guard` | ✅ synced | `main` |
| `Claude-Code-Everything-You-Need-to-Know` | ✅ synced | `main` |
| `tweakcc` | ✅ synced | `main` |
| `oh-my-claudecode` | ✅ synced | `main` |
| `awesome-cursorrules` | ✅ synced | `main` |
| `awesome-copilot` | ⚠️ can't sync because there are diverging changes; use `--force` to overwrite the destination branch  | `main` |
| `awesome-copilot-agents` | ✅ synced | `main` |
| `awesome-cursorrules-zh` | ✅ synced | `master` |
| `myclaude` | ✅ synced | `master` |
| `overstory` | ✅ synced | `main` |
| `microsoft-skills` | ✅ synced | `main` |
| `spec-kit` | ✅ synced | `main` |
| `OpenSpec` | ✅ synced | `main` |
| `spec-workflow-mcp` | ✅ synced | `main` |
| `get-shit-done` | ✅ synced | `main` |
| `shotgun` | ✅ synced | `main` |
| `agents` | ✅ synced | `main` |
| `ruflo` | ⚠️ can't sync because there are diverging changes; use `--force` to overwrite the destination branch  | `main` |
| `open-multi-agent` | ✅ synced | `main` |
| `maestro-orchestrate` | ✅ synced | `main` |
| `flow-next` | ✅ synced | `main` |
| `claude-code-workflow` | ✅ synced | `main` |
| `context-engineering-kit` | ✅ synced | `master` |
| `mcp-servers-official` | ✅ synced | `main` |
| `awesome-mcp-servers` | ✅ synced | `main` |
| `Awesome-MCP-ZH` | ✅ synced | `main` |
| `awesome-remote-mcp-servers` | ✅ synced | `main` |
| `awesome-claude-plugins` | ✅ synced | `master` |
| `awesome-mcp-servers-appcypher` | ✅ synced | `main` |
| `agents.md` | ✅ synced | `main` |
| `design.md` | ✅ synced | `main` |
| `awesome-design-md` | ✅ synced | `main` |
| `Prompt-Engineering-Guide` | ✅ synced | `main` |
| `prompts.chat` | ✅ synced | `main` |
| `SwiftAgents` | ✅ synced | `main` |
| `rails_ai_agents` | ✅ synced | `main` |
| `awesome-openclaw-agents` | ✅ synced | `main` |
| `claude-code-templates` | ⚠️ can't sync because there are diverging changes; use `--force` to overwrite the destination branch  | `main` |
| `claude-code-system-prompts` | ✅ synced | `main` |
| `claude-code-ultimate-guide` | ✅ synced | `main` |
| `fastmcp` | ✅ synced | `main` |
| `edict` | ✅ synced | `main` |
| `microsoft-agent-framework` | ✅ synced | `main` |
| `swarms` | ✅ synced | `master` |
| `mission-control` | ✅ synced | `main` |
| `skills-mattpocock` | ✅ synced | `main` |
| `claude-mem` | ✅ synced | `main` |
| `oh-my-openagent` | ✅ synced | `dev` |
| `potpie` | ✅ synced | `main` |
| `cc-sdd` | ✅ synced | `main` |
| `gsd-2` | ✅ synced | `main` |
| `github-mcp-server` | ✅ synced | `main` |
| `my-claude-code-setup` | ✅ synced | `master` |
| `claude-token-efficient` | ⚠️ can't sync because there are diverging changes; use `--force` to overwrite the destination branch  | `main` |
| `playwright-mcp` | ✅ synced | `main` |
| `mcp-toolbox` | ✅ synced | `main` |
| `awesome-codex-subagents` | ✅ synced | `main` |
| `Awesome-Context-Engineering` | ✅ synced | `main` |
| `graphify` | ✅ synced | `v8` |
| `Understand-Anything` | ✅ synced | `main` |
| `awesome-openclaw-skills` | ✅ synced | `main` |
| `gsd-core` | ✅ synced | `next` |
| `agent-os` | ✅ synced | `main` |


## 2026-06-29T08:46:01Z

**Summary**: ✅ 90 synced · ⚠️ 4 failed · ❌ 0 skipped (total 94)

| Repo | Result | Branch |
|---|---|---|
| `skills` | ✅ synced | `main` |
| `superpowers` | ✅ synced | `main` |
| `andrej-karpathy-skills` | ✅ synced | `main` |
| `gstack` | ✅ synced | `main` |
| `everything-claude-code` | ✅ synced | `main` |
| `awesome-claude-code` | ✅ synced | `main` |
| `awesome-claude-skills` | ✅ synced | `main` |
| `antigravity-awesome-skills` | ✅ synced | `main` |
| `claude-howto` | ✅ synced | `main` |
| `awesome-agent-skills` | ✅ synced | `main` |
| `awesome-claude-code-subagents` | ✅ synced | `main` |
| `ralph` | ✅ synced | `main` |
| `awesome-claude-skills-travisvn` | ✅ synced | `main` |
| `context-engineering-intro` | ✅ synced | `main` |
| `claude-code-best-practice` | ✅ synced | `main` |
| `how-claude-code-works` | ✅ synced | `main` |
| `pro-workflow` | ✅ synced | `main` |
| `claude-agents` | ✅ synced | `main` |
| `awesome-claude-code-toolkit` | ✅ synced | `main` |
| `claude-code-sub-agents` | ✅ synced | `main` |
| `claude-code-subagents` | ✅ synced | `main` |
| `awesome-claude-code-plugins` | ✅ synced | `main` |
| `awesome-claude-agents` | ✅ synced | `main` |
| `awesome-agent-skills-heilcheng` | ✅ synced | `main` |
| `awesome-claude-code-jqueryscript` | ✅ synced | `main` |
| `awesome-claude` | ✅ synced | `main` |
| `claude-code-hooks-mastery` | ✅ synced | `main` |
| `claude-code-infrastructure-showcase` | ✅ synced | `main` |
| `claude-code-showcase` | ✅ synced | `main` |
| `Continuous-Claude-v3` | ✅ synced | `main` |
| `buildwithclaude` | ✅ synced | `main` |
| `tdd-guard` | ✅ synced | `main` |
| `Claude-Code-Everything-You-Need-to-Know` | ✅ synced | `main` |
| `tweakcc` | ✅ synced | `main` |
| `oh-my-claudecode` | ✅ synced | `main` |
| `awesome-cursorrules` | ✅ synced | `main` |
| `awesome-copilot` | ⚠️ can't sync because there are diverging changes; use `--force` to overwrite the destination branch  | `main` |
| `awesome-copilot-agents` | ✅ synced | `main` |
| `awesome-cursorrules-zh` | ✅ synced | `master` |
| `myclaude` | ✅ synced | `master` |
| `overstory` | ✅ synced | `main` |
| `microsoft-skills` | ✅ synced | `main` |
| `spec-kit` | ✅ synced | `main` |
| `OpenSpec` | ✅ synced | `main` |
| `spec-workflow-mcp` | ✅ synced | `main` |
| `get-shit-done` | ✅ synced | `main` |
| `shotgun` | ✅ synced | `main` |
| `agents` | ✅ synced | `main` |
| `ruflo` | ⚠️ can't sync because there are diverging changes; use `--force` to overwrite the destination branch  | `main` |
| `open-multi-agent` | ✅ synced | `main` |
| `maestro-orchestrate` | ✅ synced | `main` |
| `flow-next` | ✅ synced | `main` |
| `claude-code-workflow` | ✅ synced | `main` |
| `context-engineering-kit` | ✅ synced | `master` |
| `mcp-servers-official` | ✅ synced | `main` |
| `awesome-mcp-servers` | ✅ synced | `main` |
| `Awesome-MCP-ZH` | ✅ synced | `main` |
| `awesome-remote-mcp-servers` | ✅ synced | `main` |
| `awesome-claude-plugins` | ✅ synced | `master` |
| `awesome-mcp-servers-appcypher` | ✅ synced | `main` |
| `agents.md` | ✅ synced | `main` |
| `design.md` | ✅ synced | `main` |
| `awesome-design-md` | ✅ synced | `main` |
| `Prompt-Engineering-Guide` | ✅ synced | `main` |
| `prompts.chat` | ✅ synced | `main` |
| `SwiftAgents` | ✅ synced | `main` |
| `rails_ai_agents` | ✅ synced | `main` |
| `awesome-openclaw-agents` | ✅ synced | `main` |
| `claude-code-templates` | ⚠️ can't sync because there are diverging changes; use `--force` to overwrite the destination branch  | `main` |
| `claude-code-system-prompts` | ✅ synced | `main` |
| `claude-code-ultimate-guide` | ✅ synced | `main` |
| `fastmcp` | ✅ synced | `main` |
| `edict` | ✅ synced | `main` |
| `microsoft-agent-framework` | ✅ synced | `main` |
| `swarms` | ✅ synced | `master` |
| `mission-control` | ✅ synced | `main` |
| `skills-mattpocock` | ✅ synced | `main` |
| `claude-mem` | ✅ synced | `main` |
| `oh-my-openagent` | ✅ synced | `dev` |
| `potpie` | ✅ synced | `main` |
| `cc-sdd` | ✅ synced | `main` |
| `gsd-2` | ✅ synced | `main` |
| `github-mcp-server` | ✅ synced | `main` |
| `my-claude-code-setup` | ✅ synced | `master` |
| `claude-token-efficient` | ⚠️ can't sync because there are diverging changes; use `--force` to overwrite the destination branch  | `main` |
| `playwright-mcp` | ✅ synced | `main` |
| `mcp-toolbox` | ✅ synced | `main` |
| `awesome-codex-subagents` | ✅ synced | `main` |
| `Awesome-Context-Engineering` | ✅ synced | `main` |
| `graphify` | ✅ synced | `v8` |
| `Understand-Anything` | ✅ synced | `main` |
| `awesome-openclaw-skills` | ✅ synced | `main` |
| `gsd-core` | ✅ synced | `next` |
| `agent-os` | ✅ synced | `main` |


## 2026-06-22T09:35:12Z

**Summary**: ✅ 89 synced · ⚠️ 5 failed · ❌ 0 skipped (total 94)

| Repo | Result | Branch |
|---|---|---|
| `skills` | ✅ synced | `main` |
| `superpowers` | ✅ synced | `main` |
| `andrej-karpathy-skills` | ✅ synced | `main` |
| `gstack` | ✅ synced | `main` |
| `everything-claude-code` | ✅ synced | `main` |
| `awesome-claude-code` | ✅ synced | `main` |
| `awesome-claude-skills` | ✅ synced | `main` |
| `antigravity-awesome-skills` | ✅ synced | `main` |
| `claude-howto` | ✅ synced | `main` |
| `awesome-agent-skills` | ✅ synced | `main` |
| `awesome-claude-code-subagents` | ✅ synced | `main` |
| `ralph` | ✅ synced | `main` |
| `awesome-claude-skills-travisvn` | ✅ synced | `main` |
| `context-engineering-intro` | ✅ synced | `main` |
| `claude-code-best-practice` | ✅ synced | `main` |
| `how-claude-code-works` | ✅ synced | `main` |
| `pro-workflow` | ✅ synced | `main` |
| `claude-agents` | ✅ synced | `main` |
| `awesome-claude-code-toolkit` | ✅ synced | `main` |
| `claude-code-sub-agents` | ✅ synced | `main` |
| `claude-code-subagents` | ✅ synced | `main` |
| `awesome-claude-code-plugins` | ✅ synced | `main` |
| `awesome-claude-agents` | ✅ synced | `main` |
| `awesome-agent-skills-heilcheng` | ✅ synced | `main` |
| `awesome-claude-code-jqueryscript` | ✅ synced | `main` |
| `awesome-claude` | ✅ synced | `main` |
| `claude-code-hooks-mastery` | ✅ synced | `main` |
| `claude-code-infrastructure-showcase` | ✅ synced | `main` |
| `claude-code-showcase` | ✅ synced | `main` |
| `Continuous-Claude-v3` | ✅ synced | `main` |
| `buildwithclaude` | ✅ synced | `main` |
| `tdd-guard` | ✅ synced | `main` |
| `Claude-Code-Everything-You-Need-to-Know` | ✅ synced | `main` |
| `tweakcc` | ✅ synced | `main` |
| `oh-my-claudecode` | ✅ synced | `main` |
| `awesome-cursorrules` | ✅ synced | `main` |
| `awesome-copilot` | ✅ synced | `main` |
| `awesome-copilot-agents` | ✅ synced | `main` |
| `awesome-cursorrules-zh` | ✅ synced | `master` |
| `myclaude` | ✅ synced | `master` |
| `overstory` | ✅ synced | `main` |
| `microsoft-skills` | ✅ synced | `main` |
| `spec-kit` | ✅ synced | `main` |
| `OpenSpec` | ✅ synced | `main` |
| `spec-workflow-mcp` | ✅ synced | `main` |
| `get-shit-done` | ✅ synced | `main` |
| `shotgun` | ✅ synced | `main` |
| `agents` | ⚠️ HTTP 401: Bad credentials (https://api.github.com/repos/glowElephant/agents/merge-upstream) Try authenticating with:  gh auth login -h github.com  | `main` |
| `ruflo` | ⚠️ can't sync because there are diverging changes; use `--force` to overwrite the destination branch  | `main` |
| `open-multi-agent` | ✅ synced | `main` |
| `maestro-orchestrate` | ✅ synced | `main` |
| `flow-next` | ✅ synced | `main` |
| `claude-code-workflow` | ✅ synced | `main` |
| `context-engineering-kit` | ✅ synced | `master` |
| `mcp-servers-official` | ✅ synced | `main` |
| `awesome-mcp-servers` | ✅ synced | `main` |
| `Awesome-MCP-ZH` | ✅ synced | `main` |
| `awesome-remote-mcp-servers` | ✅ synced | `main` |
| `awesome-claude-plugins` | ✅ synced | `master` |
| `awesome-mcp-servers-appcypher` | ✅ synced | `main` |
| `agents.md` | ✅ synced | `main` |
| `design.md` | ✅ synced | `main` |
| `awesome-design-md` | ✅ synced | `main` |
| `Prompt-Engineering-Guide` | ✅ synced | `main` |
| `prompts.chat` | ✅ synced | `main` |
| `SwiftAgents` | ✅ synced | `main` |
| `rails_ai_agents` | ✅ synced | `main` |
| `awesome-openclaw-agents` | ✅ synced | `main` |
| `claude-code-templates` | ⚠️ can't sync because there are diverging changes; use `--force` to overwrite the destination branch  | `main` |
| `claude-code-system-prompts` | ✅ synced | `main` |
| `claude-code-ultimate-guide` | ✅ synced | `main` |
| `fastmcp` | ✅ synced | `main` |
| `edict` | ✅ synced | `main` |
| `microsoft-agent-framework` | ✅ synced | `main` |
| `swarms` | ✅ synced | `master` |
| `mission-control` | ✅ synced | `main` |
| `skills-mattpocock` | ✅ synced | `main` |
| `claude-mem` | ✅ synced | `main` |
| `oh-my-openagent` | ✅ synced | `dev` |
| `potpie` | ✅ synced | `main` |
| `cc-sdd` | ✅ synced | `main` |
| `gsd-2` | ✅ synced | `main` |
| `github-mcp-server` | ✅ synced | `main` |
| `my-claude-code-setup` | ⚠️ HTTP 401: Bad credentials (https://api.github.com/repos/glowElephant/my-claude-code-setup/merge-upstream) Try authenticating with:  gh auth login -h g | `master` |
| `claude-token-efficient` | ⚠️ can't sync because there are diverging changes; use `--force` to overwrite the destination branch  | `main` |
| `playwright-mcp` | ✅ synced | `main` |
| `mcp-toolbox` | ✅ synced | `main` |
| `awesome-codex-subagents` | ✅ synced | `main` |
| `Awesome-Context-Engineering` | ✅ synced | `main` |
| `graphify` | ✅ synced | `v8` |
| `Understand-Anything` | ✅ synced | `main` |
| `awesome-openclaw-skills` | ✅ synced | `main` |
| `gsd-core` | ✅ synced | `next` |
| `agent-os` | ✅ synced | `main` |


## 2026-06-15T09:48:03Z

**Summary**: ✅ 92 synced · ⚠️ 2 failed · ❌ 0 skipped (total 94)

| Repo | Result | Branch |
|---|---|---|
| `skills` | ✅ synced | `main` |
| `superpowers` | ✅ synced | `main` |
| `andrej-karpathy-skills` | ✅ synced | `main` |
| `gstack` | ✅ synced | `main` |
| `everything-claude-code` | ✅ synced | `main` |
| `awesome-claude-code` | ✅ synced | `main` |
| `awesome-claude-skills` | ✅ synced | `main` |
| `antigravity-awesome-skills` | ✅ synced | `main` |
| `claude-howto` | ✅ synced | `main` |
| `awesome-agent-skills` | ✅ synced | `main` |
| `awesome-claude-code-subagents` | ✅ synced | `main` |
| `ralph` | ✅ synced | `main` |
| `awesome-claude-skills-travisvn` | ✅ synced | `main` |
| `context-engineering-intro` | ✅ synced | `main` |
| `claude-code-best-practice` | ✅ synced | `main` |
| `how-claude-code-works` | ✅ synced | `main` |
| `pro-workflow` | ✅ synced | `main` |
| `claude-agents` | ✅ synced | `main` |
| `awesome-claude-code-toolkit` | ✅ synced | `main` |
| `claude-code-sub-agents` | ✅ synced | `main` |
| `claude-code-subagents` | ✅ synced | `main` |
| `awesome-claude-code-plugins` | ✅ synced | `main` |
| `awesome-claude-agents` | ✅ synced | `main` |
| `awesome-agent-skills-heilcheng` | ✅ synced | `main` |
| `awesome-claude-code-jqueryscript` | ✅ synced | `main` |
| `awesome-claude` | ✅ synced | `main` |
| `claude-code-hooks-mastery` | ✅ synced | `main` |
| `claude-code-infrastructure-showcase` | ✅ synced | `main` |
| `claude-code-showcase` | ✅ synced | `main` |
| `Continuous-Claude-v3` | ✅ synced | `main` |
| `buildwithclaude` | ✅ synced | `main` |
| `tdd-guard` | ✅ synced | `main` |
| `Claude-Code-Everything-You-Need-to-Know` | ✅ synced | `main` |
| `tweakcc` | ✅ synced | `main` |
| `oh-my-claudecode` | ✅ synced | `main` |
| `awesome-cursorrules` | ✅ synced | `main` |
| `awesome-copilot` | ✅ synced | `main` |
| `awesome-copilot-agents` | ✅ synced | `main` |
| `awesome-cursorrules-zh` | ✅ synced | `master` |
| `myclaude` | ✅ synced | `master` |
| `overstory` | ✅ synced | `main` |
| `microsoft-skills` | ✅ synced | `main` |
| `spec-kit` | ✅ synced | `main` |
| `OpenSpec` | ✅ synced | `main` |
| `spec-workflow-mcp` | ✅ synced | `main` |
| `get-shit-done` | ✅ synced | `main` |
| `shotgun` | ✅ synced | `main` |
| `agents` | ✅ synced | `main` |
| `ruflo` | ⚠️ can't sync because there are diverging changes; use `--force` to overwrite the destination branch  | `main` |
| `open-multi-agent` | ✅ synced | `main` |
| `maestro-orchestrate` | ✅ synced | `main` |
| `flow-next` | ✅ synced | `main` |
| `claude-code-workflow` | ✅ synced | `main` |
| `context-engineering-kit` | ✅ synced | `master` |
| `mcp-servers-official` | ✅ synced | `main` |
| `awesome-mcp-servers` | ✅ synced | `main` |
| `Awesome-MCP-ZH` | ✅ synced | `main` |
| `awesome-remote-mcp-servers` | ✅ synced | `main` |
| `awesome-claude-plugins` | ✅ synced | `master` |
| `awesome-mcp-servers-appcypher` | ✅ synced | `main` |
| `agents.md` | ✅ synced | `main` |
| `design.md` | ✅ synced | `main` |
| `awesome-design-md` | ✅ synced | `main` |
| `Prompt-Engineering-Guide` | ✅ synced | `main` |
| `prompts.chat` | ✅ synced | `main` |
| `SwiftAgents` | ✅ synced | `main` |
| `rails_ai_agents` | ✅ synced | `main` |
| `awesome-openclaw-agents` | ✅ synced | `main` |
| `claude-code-templates` | ⚠️ can't sync because there are diverging changes; use `--force` to overwrite the destination branch  | `main` |
| `claude-code-system-prompts` | ✅ synced | `main` |
| `claude-code-ultimate-guide` | ✅ synced | `main` |
| `fastmcp` | ✅ synced | `main` |
| `edict` | ✅ synced | `main` |
| `microsoft-agent-framework` | ✅ synced | `main` |
| `swarms` | ✅ synced | `master` |
| `mission-control` | ✅ synced | `main` |
| `skills-mattpocock` | ✅ synced | `main` |
| `claude-mem` | ✅ synced | `main` |
| `oh-my-openagent` | ✅ synced | `dev` |
| `potpie` | ✅ synced | `main` |
| `cc-sdd` | ✅ synced | `main` |
| `gsd-2` | ✅ synced | `main` |
| `github-mcp-server` | ✅ synced | `main` |
| `my-claude-code-setup` | ✅ synced | `master` |
| `claude-token-efficient` | ✅ synced | `main` |
| `playwright-mcp` | ✅ synced | `main` |
| `mcp-toolbox` | ✅ synced | `main` |
| `awesome-codex-subagents` | ✅ synced | `main` |
| `Awesome-Context-Engineering` | ✅ synced | `main` |
| `graphify` | ✅ synced | `v8` |
| `Understand-Anything` | ✅ synced | `main` |
| `awesome-openclaw-skills` | ✅ synced | `main` |
| `gsd-core` | ✅ synced | `next` |
| `agent-os` | ✅ synced | `main` |


## 2026-06-08T08:41:44Z

**Summary**: ✅ 81 synced · ⚠️ 1 failed · ❌ 0 skipped (total 82)

| Repo | Result | Branch |
|---|---|---|
| `skills` | ✅ synced | `main` |
| `superpowers` | ✅ synced | `main` |
| `andrej-karpathy-skills` | ✅ synced | `main` |
| `gstack` | ✅ synced | `main` |
| `everything-claude-code` | ✅ synced | `main` |
| `awesome-claude-code` | ✅ synced | `main` |
| `awesome-claude-skills` | ✅ synced | `main` |
| `antigravity-awesome-skills` | ✅ synced | `main` |
| `claude-howto` | ✅ synced | `main` |
| `awesome-agent-skills` | ✅ synced | `main` |
| `awesome-claude-code-subagents` | ✅ synced | `main` |
| `ralph` | ✅ synced | `main` |
| `awesome-claude-skills-travisvn` | ✅ synced | `main` |
| `context-engineering-intro` | ✅ synced | `main` |
| `claude-code-best-practice` | ✅ synced | `main` |
| `how-claude-code-works` | ✅ synced | `main` |
| `pro-workflow` | ✅ synced | `main` |
| `claude-agents` | ✅ synced | `main` |
| `awesome-claude-code-toolkit` | ✅ synced | `main` |
| `claude-code-sub-agents` | ✅ synced | `main` |
| `claude-code-subagents` | ✅ synced | `main` |
| `awesome-claude-code-plugins` | ✅ synced | `main` |
| `awesome-claude-agents` | ✅ synced | `main` |
| `awesome-agent-skills-heilcheng` | ✅ synced | `main` |
| `awesome-claude-code-jqueryscript` | ✅ synced | `main` |
| `awesome-claude` | ✅ synced | `main` |
| `claude-code-hooks-mastery` | ✅ synced | `main` |
| `claude-code-infrastructure-showcase` | ✅ synced | `main` |
| `claude-code-showcase` | ✅ synced | `main` |
| `Continuous-Claude-v3` | ✅ synced | `main` |
| `buildwithclaude` | ✅ synced | `main` |
| `tdd-guard` | ✅ synced | `main` |
| `Claude-Code-Everything-You-Need-to-Know` | ✅ synced | `main` |
| `tweakcc` | ✅ synced | `main` |
| `oh-my-claudecode` | ✅ synced | `main` |
| `awesome-cursorrules` | ✅ synced | `main` |
| `awesome-copilot` | ✅ synced | `main` |
| `awesome-copilot-agents` | ✅ synced | `main` |
| `awesome-cursorrules-zh` | ✅ synced | `master` |
| `myclaude` | ✅ synced | `master` |
| `overstory` | ✅ synced | `main` |
| `microsoft-skills` | ✅ synced | `main` |
| `spec-kit` | ✅ synced | `main` |
| `OpenSpec` | ✅ synced | `main` |
| `spec-workflow-mcp` | ✅ synced | `main` |
| `get-shit-done` | ✅ synced | `main` |
| `shotgun` | ✅ synced | `main` |
| `agents` | ✅ synced | `main` |
| `ruflo` | ⚠️ can't sync because there are diverging changes; use `--force` to overwrite the destination branch  | `main` |
| `open-multi-agent` | ✅ synced | `main` |
| `maestro-orchestrate` | ✅ synced | `main` |
| `flow-next` | ✅ synced | `main` |
| `claude-code-workflow` | ✅ synced | `main` |
| `context-engineering-kit` | ✅ synced | `master` |
| `mcp-servers-official` | ✅ synced | `main` |
| `awesome-mcp-servers` | ✅ synced | `main` |
| `Awesome-MCP-ZH` | ✅ synced | `main` |
| `awesome-remote-mcp-servers` | ✅ synced | `main` |
| `awesome-claude-plugins` | ✅ synced | `master` |
| `awesome-mcp-servers-appcypher` | ✅ synced | `main` |
| `agents.md` | ✅ synced | `main` |
| `design.md` | ✅ synced | `main` |
| `awesome-design-md` | ✅ synced | `main` |
| `Prompt-Engineering-Guide` | ✅ synced | `main` |
| `prompts.chat` | ✅ synced | `main` |
| `SwiftAgents` | ✅ synced | `main` |
| `rails_ai_agents` | ✅ synced | `main` |
| `awesome-openclaw-agents` | ✅ synced | `main` |
| `claude-code-templates` | ✅ synced | `main` |
| `claude-code-system-prompts` | ✅ synced | `main` |
| `claude-code-ultimate-guide` | ✅ synced | `main` |
| `fastmcp` | ✅ synced | `main` |
| `edict` | ✅ synced | `main` |
| `microsoft-agent-framework` | ✅ synced | `main` |
| `swarms` | ✅ synced | `master` |
| `mission-control` | ✅ synced | `main` |
| `skills-mattpocock` | ✅ synced | `main` |
| `claude-mem` | ✅ synced | `main` |
| `oh-my-openagent` | ✅ synced | `dev` |
| `potpie` | ✅ synced | `main` |
| `cc-sdd` | ✅ synced | `main` |
| `gsd-2` | ✅ synced | `main` |


## 2026-06-01T09:07:11Z

**Summary**: ✅ 81 synced · ⚠️ 1 failed · ❌ 0 skipped (total 82)

| Repo | Result | Branch |
|---|---|---|
| `skills` | ✅ synced | `main` |
| `superpowers` | ✅ synced | `main` |
| `andrej-karpathy-skills` | ✅ synced | `main` |
| `gstack` | ✅ synced | `main` |
| `everything-claude-code` | ✅ synced | `main` |
| `awesome-claude-code` | ✅ synced | `main` |
| `awesome-claude-skills` | ✅ synced | `main` |
| `antigravity-awesome-skills` | ✅ synced | `main` |
| `claude-howto` | ✅ synced | `main` |
| `awesome-agent-skills` | ✅ synced | `main` |
| `awesome-claude-code-subagents` | ✅ synced | `main` |
| `ralph` | ✅ synced | `main` |
| `awesome-claude-skills-travisvn` | ✅ synced | `main` |
| `context-engineering-intro` | ✅ synced | `main` |
| `claude-code-best-practice` | ✅ synced | `main` |
| `how-claude-code-works` | ✅ synced | `main` |
| `pro-workflow` | ✅ synced | `main` |
| `claude-agents` | ✅ synced | `main` |
| `awesome-claude-code-toolkit` | ✅ synced | `main` |
| `claude-code-sub-agents` | ✅ synced | `main` |
| `claude-code-subagents` | ✅ synced | `main` |
| `awesome-claude-code-plugins` | ✅ synced | `main` |
| `awesome-claude-agents` | ✅ synced | `main` |
| `awesome-agent-skills-heilcheng` | ✅ synced | `main` |
| `awesome-claude-code-jqueryscript` | ✅ synced | `main` |
| `awesome-claude` | ✅ synced | `main` |
| `claude-code-hooks-mastery` | ✅ synced | `main` |
| `claude-code-infrastructure-showcase` | ✅ synced | `main` |
| `claude-code-showcase` | ✅ synced | `main` |
| `Continuous-Claude-v3` | ✅ synced | `main` |
| `buildwithclaude` | ✅ synced | `main` |
| `tdd-guard` | ✅ synced | `main` |
| `Claude-Code-Everything-You-Need-to-Know` | ✅ synced | `main` |
| `tweakcc` | ✅ synced | `main` |
| `oh-my-claudecode` | ✅ synced | `main` |
| `awesome-cursorrules` | ✅ synced | `main` |
| `awesome-copilot` | ✅ synced | `main` |
| `awesome-copilot-agents` | ✅ synced | `main` |
| `awesome-cursorrules-zh` | ✅ synced | `master` |
| `myclaude` | ✅ synced | `master` |
| `overstory` | ✅ synced | `main` |
| `microsoft-skills` | ✅ synced | `main` |
| `spec-kit` | ✅ synced | `main` |
| `OpenSpec` | ✅ synced | `main` |
| `spec-workflow-mcp` | ✅ synced | `main` |
| `get-shit-done` | ✅ synced | `main` |
| `shotgun` | ✅ synced | `main` |
| `agents` | ✅ synced | `main` |
| `ruflo` | ⚠️ can't sync because there are diverging changes; use `--force` to overwrite the destination branch  | `main` |
| `open-multi-agent` | ✅ synced | `main` |
| `maestro-orchestrate` | ✅ synced | `main` |
| `flow-next` | ✅ synced | `main` |
| `claude-code-workflow` | ✅ synced | `main` |
| `context-engineering-kit` | ✅ synced | `master` |
| `mcp-servers-official` | ✅ synced | `main` |
| `awesome-mcp-servers` | ✅ synced | `main` |
| `Awesome-MCP-ZH` | ✅ synced | `main` |
| `awesome-remote-mcp-servers` | ✅ synced | `main` |
| `awesome-claude-plugins` | ✅ synced | `master` |
| `awesome-mcp-servers-appcypher` | ✅ synced | `main` |
| `agents.md` | ✅ synced | `main` |
| `design.md` | ✅ synced | `main` |
| `awesome-design-md` | ✅ synced | `main` |
| `Prompt-Engineering-Guide` | ✅ synced | `main` |
| `prompts.chat` | ✅ synced | `main` |
| `SwiftAgents` | ✅ synced | `main` |
| `rails_ai_agents` | ✅ synced | `main` |
| `awesome-openclaw-agents` | ✅ synced | `main` |
| `claude-code-templates` | ✅ synced | `main` |
| `claude-code-system-prompts` | ✅ synced | `main` |
| `claude-code-ultimate-guide` | ✅ synced | `main` |
| `fastmcp` | ✅ synced | `main` |
| `edict` | ✅ synced | `main` |
| `microsoft-agent-framework` | ✅ synced | `main` |
| `swarms` | ✅ synced | `master` |
| `mission-control` | ✅ synced | `main` |
| `skills-mattpocock` | ✅ synced | `main` |
| `claude-mem` | ✅ synced | `main` |
| `oh-my-openagent` | ✅ synced | `dev` |
| `potpie` | ✅ synced | `main` |
| `cc-sdd` | ✅ synced | `main` |
| `gsd-2` | ✅ synced | `main` |


## 2026-05-27T04:25:53Z

**Summary**: ✅ 65 synced · ⚠️ 2 failed · ❌ 0 skipped (total 67)

| Repo | Result | Branch |
|---|---|---|
| `skills` | ✅ synced | `main` |
| `superpowers` | ✅ synced | `main` |
| `andrej-karpathy-skills` | ✅ synced | `main` |
| `gstack` | ✅ synced | `main` |
| `everything-claude-code` | ✅ synced | `main` |
| `awesome-claude-code` | ✅ synced | `main` |
| `awesome-claude-skills` | ✅ synced | `main` |
| `antigravity-awesome-skills` | ✅ synced | `main` |
| `claude-howto` | ✅ synced | `main` |
| `awesome-agent-skills` | ✅ synced | `main` |
| `awesome-claude-code-subagents` | ✅ synced | `main` |
| `ralph` | ✅ synced | `main` |
| `awesome-claude-skills-travisvn` | ✅ synced | `main` |
| `context-engineering-intro` | ✅ synced | `main` |
| `claude-code-best-practice` | ✅ synced | `main` |
| `how-claude-code-works` | ✅ synced | `main` |
| `pro-workflow` | ✅ synced | `main` |
| `claude-agents` | ✅ synced | `main` |
| `awesome-claude-code-toolkit` | ✅ synced | `main` |
| `claude-code-sub-agents` | ✅ synced | `main` |
| `claude-code-subagents` | ✅ synced | `main` |
| `awesome-claude-code-plugins` | ✅ synced | `main` |
| `awesome-claude-agents` | ✅ synced | `main` |
| `awesome-agent-skills-heilcheng` | ✅ synced | `main` |
| `awesome-claude-code-jqueryscript` | ✅ synced | `main` |
| `awesome-claude` | ✅ synced | `main` |
| `claude-code-hooks-mastery` | ✅ synced | `main` |
| `claude-code-infrastructure-showcase` | ✅ synced | `main` |
| `claude-code-showcase` | ✅ synced | `main` |
| `Continuous-Claude-v3` | ✅ synced | `main` |
| `buildwithclaude` | ✅ synced | `main` |
| `tdd-guard` | ✅ synced | `main` |
| `Claude-Code-Everything-You-Need-to-Know` | ✅ synced | `main` |
| `tweakcc` | ✅ synced | `main` |
| `oh-my-claudecode` | ✅ synced | `main` |
| `awesome-cursorrules` | ✅ synced | `main` |
| `awesome-copilot` | ⚠️ can't sync because there are diverging changes; use `--force` to overwrite the destination branch  | `main` |
| `awesome-copilot-agents` | ✅ synced | `main` |
| `awesome-cursorrules-zh` | ⚠️ can't sync because there are diverging changes; use `--force` to overwrite the destination branch  | `master` |
| `myclaude` | ✅ synced | `master` |
| `overstory` | ✅ synced | `main` |
| `microsoft-skills` | ✅ synced | `main` |
| `spec-kit` | ✅ synced | `main` |
| `OpenSpec` | ✅ synced | `main` |
| `spec-workflow-mcp` | ✅ synced | `main` |
| `get-shit-done` | ✅ synced | `main` |
| `shotgun` | ✅ synced | `main` |
| `agents` | ✅ synced | `main` |
| `ruflo` | ✅ synced | `main` |
| `open-multi-agent` | ✅ synced | `main` |
| `maestro-orchestrate` | ✅ synced | `main` |
| `flow-next` | ✅ synced | `main` |
| `claude-code-workflow` | ✅ synced | `main` |
| `context-engineering-kit` | ✅ synced | `master` |
| `mcp-servers-official` | ✅ synced | `main` |
| `awesome-mcp-servers` | ✅ synced | `main` |
| `Awesome-MCP-ZH` | ✅ synced | `main` |
| `awesome-remote-mcp-servers` | ✅ synced | `main` |
| `awesome-claude-plugins` | ✅ synced | `master` |
| `awesome-mcp-servers-appcypher` | ✅ synced | `main` |
| `agents.md` | ✅ synced | `main` |
| `design.md` | ✅ synced | `main` |
| `awesome-design-md` | ✅ synced | `main` |
| `Prompt-Engineering-Guide` | ✅ synced | `main` |
| `prompts.chat` | ✅ synced | `main` |
| `SwiftAgents` | ✅ synced | `main` |
| `rails_ai_agents` | ✅ synced | `main` |


## 2026-05-27T04:22:55Z

**Summary**: ✅ 48 synced · ⚠️ 19 failed · ❌ 0 skipped (total 67)

| Repo | Result | Branch |
|---|---|---|
| `skills` | ✅ synced | `main` |
| `superpowers` | ✅ synced | `main` |
| `andrej-karpathy-skills` | ✅ synced | `main` |
| `gstack` | ⚠️ Upstream commits contain workflow changes, which require the `workflow` scope or permission to merge. To request it, run: gh auth refresh -s workflow  | `main` |
| `everything-claude-code` | ⚠️ Upstream commits contain workflow changes, which require the `workflow` scope or permission to merge. To request it, run: gh auth refresh -s workflow  | `main` |
| `awesome-claude-code` | ✅ synced | `main` |
| `awesome-claude-skills` | ✅ synced | `main` |
| `antigravity-awesome-skills` | ⚠️ Upstream commits contain workflow changes, which require the `workflow` scope or permission to merge. To request it, run: gh auth refresh -s workflow  | `main` |
| `claude-howto` | ⚠️ Upstream commits contain workflow changes, which require the `workflow` scope or permission to merge. To request it, run: gh auth refresh -s workflow  | `main` |
| `awesome-agent-skills` | ✅ synced | `main` |
| `awesome-claude-code-subagents` | ✅ synced | `main` |
| `ralph` | ✅ synced | `main` |
| `awesome-claude-skills-travisvn` | ✅ synced | `main` |
| `context-engineering-intro` | ✅ synced | `main` |
| `claude-code-best-practice` | ✅ synced | `main` |
| `how-claude-code-works` | ✅ synced | `main` |
| `pro-workflow` | ⚠️ Upstream commits contain workflow changes, which require the `workflow` scope or permission to merge. To request it, run: gh auth refresh -s workflow  | `main` |
| `claude-agents` | ✅ synced | `main` |
| `awesome-claude-code-toolkit` | ✅ synced | `main` |
| `claude-code-sub-agents` | ✅ synced | `main` |
| `claude-code-subagents` | ✅ synced | `main` |
| `awesome-claude-code-plugins` | ✅ synced | `main` |
| `awesome-claude-agents` | ✅ synced | `main` |
| `awesome-agent-skills-heilcheng` | ✅ synced | `main` |
| `awesome-claude-code-jqueryscript` | ✅ synced | `main` |
| `awesome-claude` | ✅ synced | `main` |
| `claude-code-hooks-mastery` | ✅ synced | `main` |
| `claude-code-infrastructure-showcase` | ✅ synced | `main` |
| `claude-code-showcase` | ✅ synced | `main` |
| `Continuous-Claude-v3` | ✅ synced | `main` |
| `buildwithclaude` | ✅ synced | `main` |
| `tdd-guard` | ⚠️ Upstream commits contain workflow changes, which require the `workflow` scope or permission to merge. To request it, run: gh auth refresh -s workflow  | `main` |
| `Claude-Code-Everything-You-Need-to-Know` | ✅ synced | `main` |
| `tweakcc` | ⚠️ Upstream commits contain workflow changes, which require the `workflow` scope or permission to merge. To request it, run: gh auth refresh -s workflow  | `main` |
| `oh-my-claudecode` | ⚠️ Upstream commits contain workflow changes, which require the `workflow` scope or permission to merge. To request it, run: gh auth refresh -s workflow  | `main` |
| `awesome-cursorrules` | ⚠️ Upstream commits contain workflow changes, which require the `workflow` scope or permission to merge. To request it, run: gh auth refresh -s workflow  | `main` |
| `awesome-copilot` | ⚠️ HTTP 404: Not Found (https://api.github.com/repos/glowElephant/awesome-copilot/git/refs/heads/main)  | `main` |
| `awesome-copilot-agents` | ✅ synced | `main` |
| `awesome-cursorrules-zh` | ⚠️ HTTP 404: Not Found (https://api.github.com/repos/glowElephant/awesome-cursorrules-zh/git/refs/heads/master)  | `master` |
| `myclaude` | ✅ synced | `master` |
| `overstory` | ⚠️ Upstream commits contain workflow changes, which require the `workflow` scope or permission to merge. To request it, run: gh auth refresh -s workflow  | `main` |
| `microsoft-skills` | ✅ synced | `main` |
| `spec-kit` | ⚠️ Upstream commits contain workflow changes, which require the `workflow` scope or permission to merge. To request it, run: gh auth refresh -s workflow  | `main` |
| `OpenSpec` | ✅ synced | `main` |
| `spec-workflow-mcp` | ✅ synced | `main` |
| `get-shit-done` | ⚠️ Upstream commits contain workflow changes, which require the `workflow` scope or permission to merge. To request it, run: gh auth refresh -s workflow  | `main` |
| `shotgun` | ✅ synced | `main` |
| `agents` | ⚠️ Upstream commits contain workflow changes, which require the `workflow` scope or permission to merge. To request it, run: gh auth refresh -s workflow  | `main` |
| `ruflo` | ⚠️ Upstream commits contain workflow changes, which require the `workflow` scope or permission to merge. To request it, run: gh auth refresh -s workflow  | `main` |
| `open-multi-agent` | ✅ synced | `main` |
| `maestro-orchestrate` | ⚠️ Upstream commits contain workflow changes, which require the `workflow` scope or permission to merge. To request it, run: gh auth refresh -s workflow  | `main` |
| `flow-next` | ⚠️ Upstream commits contain workflow changes, which require the `workflow` scope or permission to merge. To request it, run: gh auth refresh -s workflow  | `main` |
| `claude-code-workflow` | ✅ synced | `main` |
| `context-engineering-kit` | ✅ synced | `master` |
| `mcp-servers-official` | ✅ synced | `main` |
| `awesome-mcp-servers` | ✅ synced | `main` |
| `Awesome-MCP-ZH` | ✅ synced | `main` |
| `awesome-remote-mcp-servers` | ✅ synced | `main` |
| `awesome-claude-plugins` | ✅ synced | `master` |
| `awesome-mcp-servers-appcypher` | ✅ synced | `main` |
| `agents.md` | ✅ synced | `main` |
| `design.md` | ⚠️ Upstream commits contain workflow changes, which require the `workflow` scope or permission to merge. To request it, run: gh auth refresh -s workflow  | `main` |
| `awesome-design-md` | ✅ synced | `main` |
| `Prompt-Engineering-Guide` | ✅ synced | `main` |
| `prompts.chat` | ✅ synced | `main` |
| `SwiftAgents` | ✅ synced | `main` |
| `rails_ai_agents` | ✅ synced | `main` |


## 2026-05-27T04:16:59Z

**Summary**: ✅ 48 synced · ⚠️ 19 failed · ❌ 0 skipped (total 67)

| Repo | Result | Branch |
|---|---|---|
| `skills` | ✅ synced | `main` |
| `superpowers` | ✅ synced | `main` |
| `andrej-karpathy-skills` | ✅ synced | `main` |
| `gstack` | ⚠️ Upstream commits contain workflow changes, which require the `workflow` scope or permission to merge. To request it, run: gh auth refresh -s workflow  | `main` |
| `everything-claude-code` | ⚠️ Upstream commits contain workflow changes, which require the `workflow` scope or permission to merge. To request it, run: gh auth refresh -s workflow  | `main` |
| `awesome-claude-code` | ✅ synced | `main` |
| `awesome-claude-skills` | ✅ synced | `main` |
| `antigravity-awesome-skills` | ⚠️ Upstream commits contain workflow changes, which require the `workflow` scope or permission to merge. To request it, run: gh auth refresh -s workflow  | `main` |
| `claude-howto` | ⚠️ Upstream commits contain workflow changes, which require the `workflow` scope or permission to merge. To request it, run: gh auth refresh -s workflow  | `main` |
| `awesome-agent-skills` | ✅ synced | `main` |
| `awesome-claude-code-subagents` | ✅ synced | `main` |
| `ralph` | ✅ synced | `main` |
| `awesome-claude-skills-travisvn` | ✅ synced | `main` |
| `context-engineering-intro` | ✅ synced | `main` |
| `claude-code-best-practice` | ✅ synced | `main` |
| `how-claude-code-works` | ✅ synced | `main` |
| `pro-workflow` | ⚠️ Upstream commits contain workflow changes, which require the `workflow` scope or permission to merge. To request it, run: gh auth refresh -s workflow  | `main` |
| `claude-agents` | ✅ synced | `main` |
| `awesome-claude-code-toolkit` | ✅ synced | `main` |
| `claude-code-sub-agents` | ✅ synced | `main` |
| `claude-code-subagents` | ✅ synced | `main` |
| `awesome-claude-code-plugins` | ✅ synced | `main` |
| `awesome-claude-agents` | ✅ synced | `main` |
| `awesome-agent-skills-heilcheng` | ✅ synced | `main` |
| `awesome-claude-code-jqueryscript` | ✅ synced | `main` |
| `awesome-claude` | ✅ synced | `main` |
| `claude-code-hooks-mastery` | ✅ synced | `main` |
| `claude-code-infrastructure-showcase` | ✅ synced | `main` |
| `claude-code-showcase` | ✅ synced | `main` |
| `Continuous-Claude-v3` | ✅ synced | `main` |
| `buildwithclaude` | ✅ synced | `main` |
| `tdd-guard` | ⚠️ Upstream commits contain workflow changes, which require the `workflow` scope or permission to merge. To request it, run: gh auth refresh -s workflow  | `main` |
| `Claude-Code-Everything-You-Need-to-Know` | ✅ synced | `main` |
| `tweakcc` | ⚠️ Upstream commits contain workflow changes, which require the `workflow` scope or permission to merge. To request it, run: gh auth refresh -s workflow  | `main` |
| `oh-my-claudecode` | ⚠️ Upstream commits contain workflow changes, which require the `workflow` scope or permission to merge. To request it, run: gh auth refresh -s workflow  | `main` |
| `awesome-cursorrules` | ⚠️ Upstream commits contain workflow changes, which require the `workflow` scope or permission to merge. To request it, run: gh auth refresh -s workflow  | `main` |
| `awesome-copilot` | ⚠️ HTTP 404: Not Found (https://api.github.com/repos/glowElephant/awesome-copilot/git/refs/heads/main)  | `main` |
| `awesome-copilot-agents` | ✅ synced | `main` |
| `awesome-cursorrules-zh` | ⚠️ HTTP 404: Not Found (https://api.github.com/repos/glowElephant/awesome-cursorrules-zh/git/refs/heads/master)  | `master` |
| `myclaude` | ✅ synced | `master` |
| `overstory` | ⚠️ Upstream commits contain workflow changes, which require the `workflow` scope or permission to merge. To request it, run: gh auth refresh -s workflow  | `main` |
| `microsoft-skills` | ✅ synced | `main` |
| `spec-kit` | ⚠️ Upstream commits contain workflow changes, which require the `workflow` scope or permission to merge. To request it, run: gh auth refresh -s workflow  | `main` |
| `OpenSpec` | ✅ synced | `main` |
| `spec-workflow-mcp` | ✅ synced | `main` |
| `get-shit-done` | ⚠️ Upstream commits contain workflow changes, which require the `workflow` scope or permission to merge. To request it, run: gh auth refresh -s workflow  | `main` |
| `shotgun` | ✅ synced | `main` |
| `agents` | ⚠️ Upstream commits contain workflow changes, which require the `workflow` scope or permission to merge. To request it, run: gh auth refresh -s workflow  | `main` |
| `ruflo` | ⚠️ Upstream commits contain workflow changes, which require the `workflow` scope or permission to merge. To request it, run: gh auth refresh -s workflow  | `main` |
| `open-multi-agent` | ✅ synced | `main` |
| `maestro-orchestrate` | ⚠️ Upstream commits contain workflow changes, which require the `workflow` scope or permission to merge. To request it, run: gh auth refresh -s workflow  | `main` |
| `flow-next` | ⚠️ Upstream commits contain workflow changes, which require the `workflow` scope or permission to merge. To request it, run: gh auth refresh -s workflow  | `main` |
| `claude-code-workflow` | ✅ synced | `main` |
| `context-engineering-kit` | ✅ synced | `master` |
| `mcp-servers-official` | ✅ synced | `main` |
| `awesome-mcp-servers` | ✅ synced | `main` |
| `Awesome-MCP-ZH` | ✅ synced | `main` |
| `awesome-remote-mcp-servers` | ✅ synced | `main` |
| `awesome-claude-plugins` | ✅ synced | `master` |
| `awesome-mcp-servers-appcypher` | ✅ synced | `main` |
| `agents.md` | ✅ synced | `main` |
| `design.md` | ⚠️ Upstream commits contain workflow changes, which require the `workflow` scope or permission to merge. To request it, run: gh auth refresh -s workflow  | `main` |
| `awesome-design-md` | ✅ synced | `main` |
| `Prompt-Engineering-Guide` | ✅ synced | `main` |
| `prompts.chat` | ✅ synced | `main` |
| `SwiftAgents` | ✅ synced | `main` |
| `rails_ai_agents` | ✅ synced | `main` |

Auto-generated by `scripts/sync-forks.sh`. Newest entries first.

<!-- The first real entry will appear above this line after the first run. -->
